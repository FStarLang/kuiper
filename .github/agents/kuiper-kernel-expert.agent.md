...
- **Repeat the literal tuple at the call site instead of binding it.** A `let ci = (page, (grow, (gcol, ())))` passed to a function does NOT inline — Custard keeps `ci` as a struct and reads it back field by field (FStarLang/FStar#4548). Pass the literal `(page, (grow, (gcol, ())))` directly to each consumer. If you still need `ci` for proof/rewriting, keep the binding but add `assert rewrites_to ci (page, (grow, (gcol, ())))` so both the readable name and the inlined literal coexist.
