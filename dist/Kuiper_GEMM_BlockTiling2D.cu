...
__global__
/**
  hoisted when extracting g_gemm_f32_8x8_cr
*/
static void
            ...
            uint32_t cols, float_t *gA, float_t *gB, float_t *gC)
{
    ...
    float_t *sB = (float_t *) KPR_SHMEM_AT(4U * (bm * bk));
    uint32_t num_n_tiles = cols / bn;
    uint32_t mrow = blockIdx.x / num_n_tiles;
    uint32_t mcol = blockIdx.x % num_n_tiles;
    uint32_t threadRow = threadIdx.x / (bn / 8U);
    uint32_t threadCol = threadIdx.x % (bn / 8U);
    float_t rAcol[8U];
    ...
    uint32_t bkIdx = 0U;
    for (; bkIdx < shared / bk; bkIdx++) {
        __syncthreads();
        uint32_t __anf01 = bkIdx;
        float_t *tileA = gA;
        uint32_t i1 = 0U;
        for (; i1 < bm * bk; i1 += bm / 8U * (bn / 8U) * 4U) {
            float_t local[4U];
            memset(local, 0U, 4U * sizeof(float_t));
            uint32_t row = (i1 + threadIdx.x * 4U) / bk;
            uint32_t col = (i1 + threadIdx.x * 4U) % bk;
            vec_memcpy(local,
                       tileA + shared * (mrow * bm) + __anf01 * bk +
                       shared * row + col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
                sA[(col + k) * bm + row] = local[k];
        }
        float_t *tileB = gB;
        uint32_t i = 0U;
        for (; i < bk * bn; i += bm / 8U * (bn / 8U) * 4U) {
            float_t local[4U];
            memset(local, 0U, 4U * sizeof(float_t));
            uint32_t row = (i + threadIdx.x * 4U) / bn;
            uint32_t col = (i + threadIdx.x * 4U) % bn;
            vec_memcpy(local,
        ...
        __syncthreads();
        uint32_t dotIdx = 0U;
        for (; dotIdx < bk; dotIdx++) {
            uint32_t i0 = 0U;
            for (; i0 < 8U; i0++)
                rAcol[i0] = sA[dotIdx * bm + threadRow * 8U + i0];
            uint32_t i1 = 0U;
            for (; i1 < 8U; i1++)
                rBrow[i1] = sB[dotIdx * bn + threadCol * 8U + i1];
            uint32_t resIdxM = 0U;
    ...
    for (; resIdxM < 8U; resIdxM++) {
        uint32_t resIdxN = 0U;
        for (; resIdxN < 8U; resIdxN++)
            t_tile[(blockIdx.x / (cols / bn) * bm +
                    threadIdx.x / (bn / 8U) * 8U + resIdxM) * cols +
                   blockIdx.x % (cols / bn) * bn +
                   threadIdx.x % (bn / 8U) * 8U + resIdxN]
                = beta *
                t_tile[(blockIdx.x / (cols / bn) * bm +
                        threadIdx.x / (bn / 8U) * 8U + resIdxM) * cols +
                       blockIdx.x % (cols / bn) * bn +
                                            ...
                                            float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_0,
              ...
              bm / 8U * (bn / 8U),
              4U * (bm * bk) + 4U * (bk * bn),
              bm, bn, bk, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x64x8_8x8_cr
*/
static void
            ...
            uint32_t cols, float_t *gA, float_t *gB, float_t *gC)
{
    ...
    for (; bkIdx < shared / 8U; bkIdx++) {
        ...
        for (; i1 < 512U; i1 += 256U) {
            float_t local[4U];
            memset(local, 0U, 4U * sizeof(float_t));
            uint32_t row = (i1 + threadIdx.x * 4U) / 8U;
            uint32_t col = (i1 + threadIdx.x * 4U) % 8U;
            vec_memcpy(local,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x8_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x8_8x16_cr(float_t alpha,
                                                     float_t beta,
                                                     uint32_t rows,
                                                     uint32_t shared,
                                                     uint32_t cols,
                                                     ...
                                                     float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_2,
              rows / 64U * (cols / 64U),
              32U, 4096U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x64x8_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x8_16x8_cr(float_t alpha,
                                                     float_t beta,
                                                     uint32_t rows,
                                                     uint32_t shared,
                                                     uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x8_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x8_16x16_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
                                                      ...
                                                      float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_4,
              rows / 64U * (cols / 64U),
              16U, 4096U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x64x16_8x8_cr
*/
static void
            ...
            uint32_t cols, float_t *gA, float_t *gB, float_t *gC)
{
    ...
    for (; bkIdx < shared / 16U; bkIdx++) {
        ...
        for (; i1 < 1024U; i1 += 256U) {
            float_t local[4U];
            memset(local, 0U, 4U * sizeof(float_t));
            uint32_t row = (i1 + threadIdx.x * 4U) / 16U;
            uint32_t col = (i1 + threadIdx.x * 4U) % 16U;
            vec_memcpy(local,
                                                     ...
                                                     float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_5,
              rows / 64U * (cols / 64U),
              64U, 8192U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x64x16_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x16_8x16_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
                                                      ...
                                                      float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_6,
              rows / 64U * (cols / 64U),
              32U, 8192U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x64x16_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x16_16x8_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x16_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x16_16x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x32_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x32_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x32_8x16_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x32_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x32_16x8_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x32_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x32_16x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x64_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x64_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x64_8x16_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x64_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x64_16x8_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x64x64_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x64x64_16x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x8_8x8_cr
*/
static void
                                                     ...
                                                     float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_17,
              rows / 64U * (cols / 128U),
              128U, 6144U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x128x8_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x8_8x16_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
                                                      ...
                                                      float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_18,
              rows / 64U * (cols / 128U),
              64U, 6144U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x128x8_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x8_16x8_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x8_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x8_16x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
                                                       ...
                                                       float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_20,
              rows / 64U * (cols / 128U),
              32U, 6144U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x128x16_8x8_cr
*/
static void
                                                      ...
                                                      float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_21,
              rows / 64U * (cols / 128U),
              128U, 12288U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x128x16_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x16_8x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
                                                       ...
                                                       float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_22,
              rows / 64U * (cols / 128U),
              64U, 12288U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_64x128x16_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x16_16x8_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x16_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x16_16x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x32_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x32_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x32_8x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x32_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x32_16x8_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x32_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x32_16x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x64_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x64_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x64_8x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x64_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x64_16x8_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_64x128x64_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_64x128x64_16x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x8_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x8_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x8_8x16_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x8_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x8_16x8_cr(float_t alpha,
                                                      float_t beta,
                                                      uint32_t rows,
                                                      uint32_t shared,
                                                      uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x8_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x8_16x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x16_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x16_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x16_8x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x16_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x16_16x8_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x16_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x16_16x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x32_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x32_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x32_8x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x32_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x32_16x8_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x32_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x32_16x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x64_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x64_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x64_8x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x64_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x64_16x8_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x64x64_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x64x64_16x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x8_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x8_8x16_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
                                                       ...
                                                       float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_50,
              rows / 128U * (cols / 128U),
              128U, 8192U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_128x128x8_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x8_16x8_cr(float_t alpha,
                                                       float_t beta,
                                                       uint32_t rows,
                                                       uint32_t shared,
                                                       uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x8_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x8_16x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x16_8x8_cr
*/
static void
                                                       ...
                                                       float_t *gB, float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_53,
              rows / 128U * (cols / 128U),
              256U, 16384U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_128x128x16_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x16_8x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
                                                        ...
                                                        float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_54,
              rows / 128U * (cols / 128U),
              128U, 16384U, alpha, beta, shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}

__global__
/**
  hoisted when extracting g_gemm_f32_128x128x16_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x16_16x8_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x16_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x16_16x16_cr(float_t alpha,
                                                         float_t beta,
                                                         uint32_t rows,
                                                         uint32_t shared,
                                                         uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x32_8x8_cr
*/
static void
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x32_8x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x32_8x16_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x32_16x8_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x32_16x8_cr(float_t alpha,
                                                        float_t beta,
                                                        uint32_t rows,
                                                        uint32_t shared,
                                                        uint32_t cols,
...
__global__
/**
  hoisted when extracting g_gemm_f32_128x128x32_16x16_cr
*/
static void
...
void
Kuiper_GEMM_BlockTiling2D_g_gemm_f32_128x128x32_16x16_cr(float_t alpha,
                                                         float_t beta,
                                                         uint32_t rows,
                                                         uint32_t shared,
                                                         uint32_t cols,
