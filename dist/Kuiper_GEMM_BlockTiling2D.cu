...
)
{
  ...
  for (; bkIdx < shared / 8U; bkIdx += 1U)
  {
    ...
    for (; i1 < 512U; i1 += 256U)
    {
      ...
      uint32_t row = i1 / 8U;
      uint32_t col = i1 % 8U;
      vec_memcpy(local, tileA + shared * (mrow * 64U) + __anf01 * 8U + shared * row + col);
      uint32_t k = 0U;
      for (; k < 4U; k += 1U)
    ...
    for (; i < 512U; i += 256U)
    {
      ...
      uint32_t row = i / 64U;
      uint32_t col = i % 64U;
      vec_memcpy(local, tileB + cols * (__anf01 * 8U) + mcol * 64U + cols * row + col);
      uint32_t k = 0U;
      for (; k < 4U; k += 1U)
...
)
{
  ...
  for (; bkIdx < shared / 8U; bkIdx += 1U)
  {
    ...
    for (; i1 < 1024U; i1 += 1024U)
    {
      ...
      uint32_t row = i1 / 8U;
      uint32_t col = i1 % 8U;
      vec_memcpy(local, tileA + shared * (mrow * 128U) + __anf01 * 8U + shared * row + col);
      uint32_t k = 0U;
      for (; k < 4U; k += 1U)
        sA[row * 8U + col + k] = local[k];
    }
    ...
    for (; i < 1024U; i += 1024U)
    {
      ...
      uint32_t row = i / 128U;
      uint32_t col = i % 128U;
      vec_memcpy(local, tileB + cols * (__anf01 * 8U) + mcol * 128U + cols * row + col);
      uint32_t k = 0U;
      for (; k < 4U; k += 1U)
