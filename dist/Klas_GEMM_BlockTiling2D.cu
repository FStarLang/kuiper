            ...
            uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 32U; bkIdx++) {
        ...
        for (; i < 1024U; i += 64U) {
            ...
            vec_memcpy(local,
                       gA + shared * (mrow * 32U) + bkIdx * 32U + shared * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
        ...
        for (; i0 < 1024U; i0 += 64U) {
            ...
            vec_memcpy(local,
                       gB + cols * (bkIdx * 32U) + mcol * 32U + cols * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
            ...
            uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 64U; bkIdx++) {
        ...
        for (; i < 2048U; i += 64U) {
            ...
            vec_memcpy(local,
                       gA + shared * (mrow * 32U) + bkIdx * 64U + shared * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
                sA[(col + k) * 32U + row] = local[k];
        ...
        for (; i0 < 2048U; i0 += 64U) {
            ...
            uint32_t col = (i0 + threadIdx.x * 4U) % 32U;
            vec_memcpy(local,
                       gB + cols * (bkIdx * 64U) + mcol * 32U + cols * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
                sB[row * 32U + col + k] = local[k];
            ...
            uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 32U; bkIdx++) {
        ...
        for (; i0 < 2048U; i0 += 128U) {
            ...
            vec_memcpy(local,
                       gB + cols * (bkIdx * 32U) + mcol * 64U + cols * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
             ...
             uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 64U; bkIdx++) {
        ...
        for (; i0 < 4096U; i0 += 128U) {
            ...
            uint32_t col = (i0 + threadIdx.x * 4U) % 64U;
            vec_memcpy(local,
                       gB + cols * (bkIdx * 64U) + mcol * 64U + cols * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
                sB[row * 64U + col + k] = local[k];
             ...
             uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 32U; bkIdx++) {
        ...
        for (; i0 < 4096U; i0 += 256U) {
            float local[4U];
            memset(local, 0U, 4U * sizeof(float));
            uint32_t row = (i0 + threadIdx.x * 4U) / 128U;
            uint32_t col = (i0 + threadIdx.x * 4U) % 128U;
            vec_memcpy(local,
                       gB + cols * (bkIdx * 32U) + mcol * 128U + cols * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
             ...
             uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 64U; bkIdx++) {
        ...
        for (; i0 < 8192U; i0 += 256U) {
            ...
            uint32_t col = (i0 + threadIdx.x * 4U) % 128U;
            vec_memcpy(local,
                       gB + cols * (bkIdx * 64U) + mcol * 128U + cols * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
                sB[row * 128U + col + k] = local[k];
             ...
             uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 32U; bkIdx++) {
        ...
        for (; i < 2048U; i += 128U) {
            ...
            uint32_t col = (i + threadIdx.x * 4U) % 32U;
            vec_memcpy(local,
                       gA + shared * (mrow * 64U) + bkIdx * 32U + shared * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
             ...
             uint32_t shared, uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    for (; bkIdx < shared / 64U; bkIdx++) {
        ...
        for (; i < 4096U; i += 128U) {
            ...
            uint32_t col = (i + threadIdx.x * 4U) % 64U;
            vec_memcpy(local,
                       gA + shared * (mrow * 64U) + bkIdx * 64U + shared * row +
                       col);
            uint32_t k = 0U;
            for (; k < 4U; k++)
                sA[(col + k) * 64U + row] = local[k];
