...
void Kuiper_Example_TensorCore_test(half_t *m1, half_t *m2, half_t *m3)
{
  ...
  wmma::load_matrix_sync(fa, (m1 + (uint32_t)0U), (uint32_t)16U);
  wmma::load_matrix_sync(fb, (m2 + (uint32_t)0U), (uint32_t)16U);
  wmma::fill_fragment(fc, (half_t)0.0f);
  wmma::mma_sync(fc, fa, fb, fc);
  wmma::store_matrix_sync((m3 + (uint32_t)0U), fc, (uint32_t)16U, wmma::mem_row_major);
}
...
void Kuiper_Example_TensorCore_test2(half_t *m1, half_t *m2, half_t *m3)
{
  ...
  half_t *t2 = m2;
  half_t *t3 = m3;
  wmma::load_matrix_sync(fa, (m1 + (uint32_t)784U), (uint32_t)48U);
  wmma::load_matrix_sync(fb, (t2 + (uint32_t)784U), (uint32_t)48U);
  wmma::fill_fragment(fc, (half_t)0.0f);
  wmma::mma_sync(fc, fa, fb, fc);
  wmma::store_matrix_sync((t3 + (uint32_t)784U),
    fc,
    (uint32_t)48U,
    wmma::mem_row_major);
}
