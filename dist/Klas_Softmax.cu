...
static void __hoisted_0(uint32_t lena, half *a)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] =
            hexp(a[1024U * blockIdx.x + threadIdx.x]);
}
...
static void __hoisted_2(uint32_t lena, half *a, half sum)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] =
            __hdiv(a[1024U * blockIdx.x + threadIdx.x], sum);
}

void Klas_Softmax_softmax_gpu_n_f16(uint32_t nth, uint32_t lena, half *a)
{
    KPR_KCALL(__hoisted_0, (lena + 1023U) / 1024U, 1024U, 0U, lena, a);
...
*/
static void __hoisted_3(uint32_t lena, float *a)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] =
            expf(a[1024U * blockIdx.x + threadIdx.x]);
}
...
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] /= sum;
}

void Klas_Softmax_softmax_gpu_n_f32(uint32_t nth, uint32_t lena, float *a)
{
    KPR_KCALL(__hoisted_3, (lena + 1023U) / 1024U, 1024U, 0U, lena, a);
    MUST(cudaDeviceSynchronize());
...
*/
static void __hoisted_6(uint32_t lena, double *a)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] =
            exp(a[1024U * blockIdx.x + threadIdx.x]);
}
...
{
    KPR_KCALL(__hoisted_6, (lena + 1023U) / 1024U, 1024U, 0U, lena, a);
    MUST(cudaDeviceSynchronize());
...
*/
static void __hoisted_9(uint32_t lena, half *a)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] =
            hexp(a[1024U * blockIdx.x + threadIdx.x]);
...
{
    KPR_KCALL(__hoisted_9, (lena + 1023U) / 1024U, 1024U, 0U, lena, a);
    MUST(cudaDeviceSynchronize());
    half *a_ = (half *) KPR_GPU_ALLOC(2U, lena);
    MUST(cudaMemcpy(a_, a, 2U * lena, cudaMemcpyDeviceToDevice));
    half *out = (half *) KPR_GPU_ALLOC(2U, 1U);
...
*/
static void __hoisted_12(uint32_t lena, float *a)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] =
            expf(a[1024U * blockIdx.x + threadIdx.x]);
...
{
    KPR_KCALL(__hoisted_12, (lena + 1023U) / 1024U, 1024U, 0U, lena, a);
    MUST(cudaDeviceSynchronize());
    float *a_ = (float *)KPR_GPU_ALLOC(4U, lena);
    MUST(cudaMemcpy(a_, a, 4U * lena, cudaMemcpyDeviceToDevice));
    float *out = (float *)KPR_GPU_ALLOC(4U, 1U);
...
*/
static void __hoisted_15(uint32_t lena, double *a)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        a[1024U * blockIdx.x + threadIdx.x] =
            exp(a[1024U * blockIdx.x + threadIdx.x]);
...
{
    KPR_KCALL(__hoisted_15, (lena + 1023U) / 1024U, 1024U, 0U, lena, a);
    MUST(cudaDeviceSynchronize());
    double *a_ = (double *)KPR_GPU_ALLOC(8U, lena);
    MUST(cudaMemcpy(a_, a, 8U * lena, cudaMemcpyDeviceToDevice));
    double *out = (double *)KPR_GPU_ALLOC(8U, 1U);
...
*/
static void __hoisted_18(uint32_t lena, half *ga)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
...
void Klas_Softmax_softmax_n_f16(uint32_t nth, uint32_t lena, half *a)
{
    ...
    KPR_KCALL(__hoisted_18, (lena + 1023U) / 1024U, 1024U, 0U, lena, ga);
    MUST(cudaDeviceSynchronize());
    half *a_ = (half *) KPR_GPU_ALLOC(2U, lena);
...
*/
static void __hoisted_21(uint32_t lena, float *ga)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        ga[1024U * blockIdx.x + threadIdx.x] =
...
void Klas_Softmax_softmax_n_f32(uint32_t nth, uint32_t lena, float *a)
{
    ...
    KPR_KCALL(__hoisted_21, (lena + 1023U) / 1024U, 1024U, 0U, lena, ga);
    MUST(cudaDeviceSynchronize());
    float *a_ = (float *)KPR_GPU_ALLOC(4U, lena);
...
*/
static void __hoisted_24(uint32_t lena, double *ga)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        ga[1024U * blockIdx.x + threadIdx.x] =
...
void Klas_Softmax_softmax_n_f64(uint32_t nth, uint32_t lena, double *a)
{
    ...
    KPR_KCALL(__hoisted_24, (lena + 1023U) / 1024U, 1024U, 0U, lena, ga);
    MUST(cudaDeviceSynchronize());
    double *a_ = (double *)KPR_GPU_ALLOC(8U, lena);
...
*/
static void __hoisted_27(uint32_t lena, half *ga)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        ga[1024U * blockIdx.x + threadIdx.x] =
            hexp(ga[1024U * blockIdx.x + threadIdx.x]);
...
void Klas_Softmax_softmax_f16(uint32_t lena, half *a)
{
    half *ga = (half *) KPR_GPU_ALLOC(2U, lena);
    MUST(cudaMemcpy(ga, a, 2U * lena, cudaMemcpyHostToDevice));
    KPR_KCALL(__hoisted_27, (lena + 1023U) / 1024U, 1024U, 0U, lena, ga);
    MUST(cudaDeviceSynchronize());
    half *a_ = (half *) KPR_GPU_ALLOC(2U, lena);
    MUST(cudaMemcpy(a_, ga, 2U * lena, cudaMemcpyDeviceToDevice));
    half *out = (half *) KPR_GPU_ALLOC(2U, 1U);
...
*/
static void __hoisted_30(uint32_t lena, float *ga)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        ga[1024U * blockIdx.x + threadIdx.x] =
            expf(ga[1024U * blockIdx.x + threadIdx.x]);
...
void Klas_Softmax_softmax_f32(uint32_t lena, float *a)
{
    float *ga = (float *)KPR_GPU_ALLOC(4U, lena);
    MUST(cudaMemcpy(ga, a, 4U * lena, cudaMemcpyHostToDevice));
    KPR_KCALL(__hoisted_30, (lena + 1023U) / 1024U, 1024U, 0U, lena, ga);
    MUST(cudaDeviceSynchronize());
    float *a_ = (float *)KPR_GPU_ALLOC(4U, lena);
    MUST(cudaMemcpy(a_, ga, 4U * lena, cudaMemcpyDeviceToDevice));
    float *out = (float *)KPR_GPU_ALLOC(4U, 1U);
...
*/
static void __hoisted_33(uint32_t lena, double *ga)
{
    if (1024U * blockIdx.x + threadIdx.x < lena)
        ga[1024U * blockIdx.x + threadIdx.x] =
            exp(ga[1024U * blockIdx.x + threadIdx.x]);
...
void Klas_Softmax_softmax_f64(uint32_t lena, double *a)
{
    double *ga = (double *)KPR_GPU_ALLOC(8U, lena);
    MUST(cudaMemcpy(ga, a, 8U * lena, cudaMemcpyHostToDevice));
    KPR_KCALL(__hoisted_33, (lena + 1023U) / 1024U, 1024U, 0U, lena, ga);
    MUST(cudaDeviceSynchronize());
    double *a_ = (double *)KPR_GPU_ALLOC(8U, lena);
    MUST(cudaMemcpy(a_, ga, 8U * lena, cudaMemcpyDeviceToDevice));
    double *out = (double *)KPR_GPU_ALLOC(8U, 1U);
