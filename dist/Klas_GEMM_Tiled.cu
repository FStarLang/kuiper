            ...
            float *gA, float *gB, float *gC, uint32_t nn, uint32_t kk)
{
    ...
    for (; bk < kk; bk++) {
        ...
        for (; k1 < tile; k1++)
            sum1 +=
                gA[(tile * (blockIdx.x / nn) + threadIdx.x / tile) * k +
                   tile * bk + k1] * gB[(tile * bk + k1) * n +
                                        tile * (blockIdx.x % nn) +
                                        threadIdx.x % tile];
        float s_ = sum1;
        sum += s_;
            ...
            uint32_t nn, uint32_t kk)
{
    ...
    for (; bk < kk; bk++) {
        ...
        for (; k1 < 32U; k1++)
            sum1 +=
                gA[(32U * (blockIdx.x / nn) + threadIdx.x / 32U) * k +
                   32U * bk + k1] * gB[(32U * bk + k1) * n +
                                       32U * (blockIdx.x % nn) +
                                       threadIdx.x % 32U];
        float s_ = sum1;
        sum += s_;
            ...
            uint32_t nn, uint32_t kk)
{
    ...
    for (; bk < kk; bk++) {
        ...
        for (; k1 < 16U; k1++)
            sum1 +=
                gA[(16U * (blockIdx.x / nn) + threadIdx.x / 16U) * k +
                   16U * bk + k1] * gB[(16U * bk + k1) * n +
                                       16U * (blockIdx.x % nn) +
                                       threadIdx.x % 16U];
        float s_ = sum1;
        sum += s_;
