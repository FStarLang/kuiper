...
void Klas_LogSoftmax_log_softmax_gpu_n_f16(uint32_t nth, uint32_t lena, half *a)
{
    ...
    half sum = hout;
    MUST(cudaFree(a_));
    KPR_KCALL(__hoisted_1, lena / 1024U + (lena % 1024U != 0U ? 1U : 0U), 1024U,
              0U, lena, a, sum);
    MUST(cudaDeviceSynchronize());
}
                                           ...
                                           float *a)
{
    ...
    float sum = hout;
    MUST(cudaFree(a_));
    KPR_KCALL(__hoisted_3, lena / 1024U + (lena % 1024U != 0U ? 1U : 0U), 1024U,
              0U, lena, a, sum);
    MUST(cudaDeviceSynchronize());
}
                                           ...
                                           double *a)
{
    ...
    double sum = hout;
    MUST(cudaFree(a_));
    KPR_KCALL(__hoisted_5, lena / 1024U + (lena % 1024U != 0U ? 1U : 0U), 1024U,
              0U, lena, a, sum);
    MUST(cudaDeviceSynchronize());
}
...
void Klas_LogSoftmax_log_softmax_gpu_f16(uint32_t lena, half *a)
{
    ...
    half sum = hout;
    MUST(cudaFree(a_));
    KPR_KCALL(__hoisted_7, lena / 1024U + (lena % 1024U != 0U ? 1U : 0U), 1024U,
              0U, lena, a, sum);
    MUST(cudaDeviceSynchronize());
}
...
void Klas_LogSoftmax_log_softmax_gpu_f32(uint32_t lena, float *a)
{
    ...
    float sum = hout;
    MUST(cudaFree(a_));
    KPR_KCALL(__hoisted_9, lena / 1024U + (lena % 1024U != 0U ? 1U : 0U), 1024U,
              0U, lena, a, sum);
    MUST(cudaDeviceSynchronize());
}
...
void Klas_LogSoftmax_log_softmax_gpu_f64(uint32_t lena, double *a)
{
    ...
    KPR_KCALL(__hoisted_11,
              lena / 1024U + (lena % 1024U != 0U ? 1U : 0U),
              1024U, 0U, lena, a, sum);
    MUST(cudaDeviceSynchronize());
}
