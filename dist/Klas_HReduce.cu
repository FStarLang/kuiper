...
*/
static void
__hoisted_reduce_f16_plus_0(uint32_t nth, uint32_t lena, half *x_, half *out)
{
...
half Klas_HReduce_reduce_f16_plus(uint32_t nth, uint32_t lena, half *a)
{
    ...
    KPR_KCALL(
        __hoisted_reduce_f16_plus_0, 1U, nth, 2U * nth, s, nth, lena, x_, out);
...
float Klas_HReduce_reduce_f32_plus(uint32_t nth, uint32_t lena, float *a)
{
    ...
    KPR_KCALL(
        __hoisted_reduce_f32_plus_0, 1U, nth, 4U * nth, s, nth, lena, x_, out);
    MUST(cudaStreamSynchronize(s));
    MUST(cudaStreamDestroy(s));
...
double Klas_HReduce_reduce_f64_plus(uint32_t nth, uint32_t lena, double *a)
{
    ...
    KPR_KCALL(
        __hoisted_reduce_f64_plus_0, 1U, nth, 8U * nth, s, nth, lena, x_, out);
    MUST(cudaStreamSynchronize(s));
    MUST(cudaStreamDestroy(s));
...
__hoisted_reduce_u32_plus_0(
    uint32_t nth, uint32_t lena, uint32_t *x_, uint32_t *out)
{
...
uint32_t Klas_HReduce_reduce_u32_plus(uint32_t nth, uint32_t lena, uint32_t *a)
{
    ...
    if (4U * nth >= 49152U)
        MUST(cudaFuncSetAttribute(__hoisted_reduce_u32_plus_0,
            cudaFuncAttributeMaxDynamicSharedMemorySize, 4U * nth));
    KPR_KCALL(
        __hoisted_reduce_u32_plus_0, 1U, nth, 4U * nth, s, nth, lena, x_, out);
    MUST(cudaStreamSynchronize(s));
    MUST(cudaStreamDestroy(s));
...
__hoisted_reduce_u64_plus_0(
    uint32_t nth, uint32_t lena, uint64_t *x_, uint64_t *out)
{
...
uint64_t Klas_HReduce_reduce_u64_plus(uint32_t nth, uint32_t lena, uint64_t *a)
{
    ...
    if (8U * nth >= 49152U)
        MUST(cudaFuncSetAttribute(__hoisted_reduce_u64_plus_0,
            cudaFuncAttributeMaxDynamicSharedMemorySize, 8U * nth));
    KPR_KCALL(
        __hoisted_reduce_u64_plus_0, 1U, nth, 8U * nth, s, nth, lena, x_, out);
    MUST(cudaStreamSynchronize(s));
    MUST(cudaStreamDestroy(s));
