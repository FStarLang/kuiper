/* Generated by F* Custard extraction. Do not edit. */
#include "Klas_HReduce.h"

__global__ static void Klas_HReduce_reduce_f16_plus_kernel_0(
    uint32_t lena, custard_f16 *x_, uint32_t nth, custard_f16 *out);
__global__ static void Klas_HReduce_reduce_f32_plus_kernel_1(
    uint32_t lena, float *x_, uint32_t nth, float *out);
__global__ static void Klas_HReduce_reduce_f64_plus_kernel_2(
    uint32_t lena, double *x_, uint32_t nth, double *out);
__global__ static void Klas_HReduce_reduce_u32_plus_kernel_3(
    uint32_t lena, uint32_t *x_, uint32_t nth, uint32_t *out);
__global__ static void Klas_HReduce_reduce_u64_plus_kernel_4(
    uint32_t lena, uint64_t *x_, uint32_t nth, uint64_t *out);

/* hoisted by the Custard Kuiper rule */
__global__ static void Klas_HReduce_reduce_f16_plus_kernel_0(
    uint32_t lena, custard_f16 *x_, uint32_t nth, custard_f16 *out)
{
    uint8_t *shmem_base = (uint8_t *) KPR_SHMEM_AT(((uint32_t) 0U));
    FStar_Pervasives_Native_tuple2__float16_ptr_unit shmem =
        (FStar_Pervasives_Native_tuple2__float16_ptr_unit) {
            ._1 = (custard_f16 *) (shmem_base + 0)};
    uint32_t bid = blockIdx.x;
    uint32_t tid = threadIdx.x;
    custard_f16 acc = CUSTARD_F16_LIT(0U);
    uint32_t idx = tid;
    while (idx < lena) {
        uint32_t idx_raw = idx;
        custard_f16 v = x_[idx_raw];
        custard_f16 __anf0 = acc;
        acc = custard_f16_add(__anf0, v);
        uint32_t __anf01 = idx;
        idx = (__anf01 + nth);
    }
    shmem._1[tid] = acc;
    uint32_t n = 0;
    while ((uint32_t) (((uint32_t) 1U) << (uint32_t) n) < nth) {
        uint32_t __anf0_1 = n;
        __syncthreads();
        uint32_t nextid =
            (tid + (uint32_t) (((uint32_t) 1U) << (uint32_t) __anf0_1));
        if (nextid < nth) {
            bool b = ((tid & ((uint32_t) (((uint32_t) 1U)
                                          << (uint32_t) (__anf0_1 + 1)) -
                                 1)) == 0);
            if (b) {
                custard_f16 s1 = shmem._1[tid];
                custard_f16 s2 = shmem._1[nextid];
                custard_f16 s = custard_f16_add(s1, s2);
                shmem._1[tid] = s;
            }
        }
        uint32_t __anf01_1 = n;
        n = (__anf01_1 + 1);
    }
    if (tid == 0) {
        uint32_t ni = 0;
        custard_f16 res = shmem._1[ni];
        out[bid] = res;
    }
}

/* hoisted by the Custard Kuiper rule */
__global__ static void Klas_HReduce_reduce_f32_plus_kernel_1(
    uint32_t lena, float *x_, uint32_t nth, float *out)
{
    uint8_t *shmem_base = (uint8_t *) KPR_SHMEM_AT(((uint32_t) 0U));
    FStar_Pervasives_Native_tuple2__float32_ptr_unit shmem =
        (FStar_Pervasives_Native_tuple2__float32_ptr_unit) {
            ._1 = (float *) (shmem_base + 0)};
    uint32_t bid = blockIdx.x;
    uint32_t tid = threadIdx.x;
    float acc = 0.0f;
    uint32_t idx = tid;
    while (idx < lena) {
        uint32_t idx_raw = idx;
        float v = x_[idx_raw];
        float __anf0 = acc;
        acc = (__anf0 + v);
        uint32_t __anf01 = idx;
        idx = (__anf01 + nth);
    }
    shmem._1[tid] = acc;
    uint32_t n = 0;
    while ((uint32_t) (((uint32_t) 1U) << (uint32_t) n) < nth) {
        uint32_t __anf0_1 = n;
        __syncthreads();
        uint32_t nextid =
            (tid + (uint32_t) (((uint32_t) 1U) << (uint32_t) __anf0_1));
        if (nextid < nth) {
            bool b = ((tid & ((uint32_t) (((uint32_t) 1U)
                                          << (uint32_t) (__anf0_1 + 1)) -
                                 1)) == 0);
            if (b) {
                float s1 = shmem._1[tid];
                float s2 = shmem._1[nextid];
                float s = (s1 + s2);
                shmem._1[tid] = s;
            }
        }
        uint32_t __anf01_1 = n;
        n = (__anf01_1 + 1);
    }
    if (tid == 0) {
        uint32_t ni = 0;
        float res = shmem._1[ni];
        out[bid] = res;
    }
}

/* hoisted by the Custard Kuiper rule */
__global__ static void Klas_HReduce_reduce_f64_plus_kernel_2(
    uint32_t lena, double *x_, uint32_t nth, double *out)
{
    uint8_t *shmem_base = (uint8_t *) KPR_SHMEM_AT(((uint32_t) 0U));
    FStar_Pervasives_Native_tuple2__float64_ptr_unit shmem =
        (FStar_Pervasives_Native_tuple2__float64_ptr_unit) {
            ._1 = (double *) (shmem_base + 0)};
    uint32_t bid = blockIdx.x;
    uint32_t tid = threadIdx.x;
    double acc = 0.0;
    uint32_t idx = tid;
    while (idx < lena) {
        uint32_t idx_raw = idx;
        double v = x_[idx_raw];
        double __anf0 = acc;
        acc = (__anf0 + v);
        uint32_t __anf01 = idx;
        idx = (__anf01 + nth);
    }
    shmem._1[tid] = acc;
    uint32_t n = 0;
    while ((uint32_t) (((uint32_t) 1U) << (uint32_t) n) < nth) {
        uint32_t __anf0_1 = n;
        __syncthreads();
        uint32_t nextid =
            (tid + (uint32_t) (((uint32_t) 1U) << (uint32_t) __anf0_1));
        if (nextid < nth) {
            bool b = ((tid & ((uint32_t) (((uint32_t) 1U)
                                          << (uint32_t) (__anf0_1 + 1)) -
                                 1)) == 0);
            if (b) {
                double s1 = shmem._1[tid];
                double s2 = shmem._1[nextid];
                double s = (s1 + s2);
                shmem._1[tid] = s;
            }
        }
        uint32_t __anf01_1 = n;
        n = (__anf01_1 + 1);
    }
    if (tid == 0) {
        uint32_t ni = 0;
        double res = shmem._1[ni];
        out[bid] = res;
    }
}

/* hoisted by the Custard Kuiper rule */
__global__ static void Klas_HReduce_reduce_u32_plus_kernel_3(
    uint32_t lena, uint32_t *x_, uint32_t nth, uint32_t *out)
{
    uint8_t *shmem_base = (uint8_t *) KPR_SHMEM_AT(((uint32_t) 0U));
    FStar_Pervasives_Native_tuple2__uint32_ptr_unit shmem =
        (FStar_Pervasives_Native_tuple2__uint32_ptr_unit) {
            ._1 = (uint32_t *) (shmem_base + 0)};
    uint32_t bid = blockIdx.x;
    uint32_t tid = threadIdx.x;
    uint32_t acc = 0;
    uint32_t idx = tid;
    while (idx < lena) {
        uint32_t idx_raw = idx;
        uint32_t v = x_[idx_raw];
        uint32_t __anf0 = acc;
        acc = (__anf0 + v);
        uint32_t __anf01 = idx;
        idx = (__anf01 + nth);
    }
    shmem._1[tid] = acc;
    uint32_t n = 0;
    while ((uint32_t) (((uint32_t) 1U) << (uint32_t) n) < nth) {
        uint32_t __anf0_1 = n;
        __syncthreads();
        uint32_t nextid =
            (tid + (uint32_t) (((uint32_t) 1U) << (uint32_t) __anf0_1));
        if (nextid < nth) {
            bool b = ((tid & ((uint32_t) (((uint32_t) 1U)
                                          << (uint32_t) (__anf0_1 + 1)) -
                                 1)) == 0);
            if (b) {
                uint32_t s1 = shmem._1[tid];
                uint32_t s2 = shmem._1[nextid];
                uint32_t s = (s1 + s2);
                shmem._1[tid] = s;
            }
        }
        uint32_t __anf01_1 = n;
        n = (__anf01_1 + 1);
    }
    if (tid == 0) {
        uint32_t ni = 0;
        uint32_t res = shmem._1[ni];
        out[bid] = res;
    }
}

/* hoisted by the Custard Kuiper rule */
__global__ static void Klas_HReduce_reduce_u64_plus_kernel_4(
    uint32_t lena, uint64_t *x_, uint32_t nth, uint64_t *out)
{
    uint8_t *shmem_base = (uint8_t *) KPR_SHMEM_AT(((uint32_t) 0U));
    FStar_Pervasives_Native_tuple2__uint64_ptr_unit shmem =
        (FStar_Pervasives_Native_tuple2__uint64_ptr_unit) {
            ._1 = (uint64_t *) (shmem_base + 0)};
    uint32_t bid = blockIdx.x;
    uint32_t tid = threadIdx.x;
    uint64_t acc = 0;
    uint32_t idx = tid;
    while (idx < lena) {
        uint32_t idx_raw = idx;
        uint64_t v = x_[idx_raw];
        uint64_t __anf0 = acc;
        acc = (__anf0 + v);
        uint32_t __anf01 = idx;
        idx = (__anf01 + nth);
    }
    shmem._1[tid] = acc;
    uint32_t n = 0;
    while ((uint32_t) (((uint32_t) 1U) << (uint32_t) n) < nth) {
        uint32_t __anf0_1 = n;
        __syncthreads();
        uint32_t nextid =
            (tid + (uint32_t) (((uint32_t) 1U) << (uint32_t) __anf0_1));
        if (nextid < nth) {
            bool b = ((tid & ((uint32_t) (((uint32_t) 1U)
                                          << (uint32_t) (__anf0_1 + 1)) -
                                 1)) == 0);
            if (b) {
                uint64_t s1 = shmem._1[tid];
                uint64_t s2 = shmem._1[nextid];
                uint64_t s = (s1 + s2);
                shmem._1[tid] = s;
            }
        }
        uint32_t __anf01_1 = n;
        n = (__anf01_1 + 1);
    }
    if (tid == 0) {
        uint32_t ni = 0;
        uint64_t res = shmem._1[ni];
        out[bid] = res;
    }
}

custard_f16 Klas_HReduce_reduce_f16_plus(
    uint32_t nth, uint32_t lena, custard_f16 *a)
{
    uint64_t lena64 = (uint64_t) lena;
    uint64_t nth64 = (uint64_t) nth;
    KPR_ASSERT((!((lena64 + nth64) < lena64)));
    uint8_t *tmp = (uint8_t *) KPR_GPU_ALLOC(((uint32_t) 2U), ((uint32_t) 1U));
    custard_f16 *out0 = (custard_f16 *) tmp;
    cudaStream_t s = KPR_FRESH_STREAM();
    KPR_SHMEM_FITS((0 + (2 * nth)));
    KPR_SET_MAX_DYN_SHMEM(
        Klas_HReduce_reduce_f16_plus_kernel_0, (0 + (2 * nth)));
    KPR_KCALL(Klas_HReduce_reduce_f16_plus_kernel_0, ((uint32_t) 1U), nth,
        (0 + (2 * nth)), s, lena, a, nth, out0);
    KPR_MUST_stream_sync(s);
    KPR_MUST_stream_destroy(s);
    custard_f16 *local_out = (custard_f16 *) malloc(1 * sizeof(custard_f16));
    if (local_out == NULL) {
        abort();
    }
    for (size_t _ci1 = 0; _ci1 < (size_t) 1; _ci1++) {
        local_out[_ci1] = CUSTARD_F16_LIT(0U);
    }
    KPR_MEMCPY_D2H(local_out, out0, (((uint32_t) 2U) * ((uint32_t) 1U)));
    custard_f16 res = local_out[0];
    free(local_out);
    KPR_GPU_FREE(out0);
    return res;
}

float Klas_HReduce_reduce_f32_plus(uint32_t nth, uint32_t lena, float *a)
{
    uint64_t lena64 = (uint64_t) lena;
    uint64_t nth64 = (uint64_t) nth;
    KPR_ASSERT((!((lena64 + nth64) < lena64)));
    uint8_t *tmp = (uint8_t *) KPR_GPU_ALLOC(((uint32_t) 4U), ((uint32_t) 1U));
    float *out0 = (float *) tmp;
    cudaStream_t s = KPR_FRESH_STREAM();
    KPR_SHMEM_FITS((0 + (4 * nth)));
    KPR_SET_MAX_DYN_SHMEM(
        Klas_HReduce_reduce_f32_plus_kernel_1, (0 + (4 * nth)));
    KPR_KCALL(Klas_HReduce_reduce_f32_plus_kernel_1, ((uint32_t) 1U), nth,
        (0 + (4 * nth)), s, lena, a, nth, out0);
    KPR_MUST_stream_sync(s);
    KPR_MUST_stream_destroy(s);
    float *local_out = (float *) malloc(1 * sizeof(float));
    if (local_out == NULL) {
        abort();
    }
    for (size_t _ci1 = 0; _ci1 < (size_t) 1; _ci1++) {
        local_out[_ci1] = 0.0f;
    }
    KPR_MEMCPY_D2H(local_out, out0, (((uint32_t) 4U) * ((uint32_t) 1U)));
    float res = local_out[0];
    free(local_out);
    KPR_GPU_FREE(out0);
    return res;
}

double Klas_HReduce_reduce_f64_plus(uint32_t nth, uint32_t lena, double *a)
{
    uint64_t lena64 = (uint64_t) lena;
    uint64_t nth64 = (uint64_t) nth;
    KPR_ASSERT((!((lena64 + nth64) < lena64)));
    uint8_t *tmp = (uint8_t *) KPR_GPU_ALLOC(((uint32_t) 8U), ((uint32_t) 1U));
    double *out0 = (double *) tmp;
    cudaStream_t s = KPR_FRESH_STREAM();
    KPR_SHMEM_FITS((0 + (8 * nth)));
    KPR_SET_MAX_DYN_SHMEM(
        Klas_HReduce_reduce_f64_plus_kernel_2, (0 + (8 * nth)));
    KPR_KCALL(Klas_HReduce_reduce_f64_plus_kernel_2, ((uint32_t) 1U), nth,
        (0 + (8 * nth)), s, lena, a, nth, out0);
    KPR_MUST_stream_sync(s);
    KPR_MUST_stream_destroy(s);
    double *local_out = (double *) malloc(1 * sizeof(double));
    if (local_out == NULL) {
        abort();
    }
    for (size_t _ci1 = 0; _ci1 < (size_t) 1; _ci1++) {
        local_out[_ci1] = 0.0;
    }
    KPR_MEMCPY_D2H(local_out, out0, (((uint32_t) 8U) * ((uint32_t) 1U)));
    double res = local_out[0];
    free(local_out);
    KPR_GPU_FREE(out0);
    return res;
}

uint32_t Klas_HReduce_reduce_u32_plus(uint32_t nth, uint32_t lena, uint32_t *a)
{
    uint64_t lena64 = (uint64_t) lena;
    uint64_t nth64 = (uint64_t) nth;
    KPR_ASSERT((!((lena64 + nth64) < lena64)));
    uint8_t *tmp = (uint8_t *) KPR_GPU_ALLOC(((uint32_t) 4U), ((uint32_t) 1U));
    uint32_t *out0 = (uint32_t *) tmp;
    cudaStream_t s = KPR_FRESH_STREAM();
    KPR_SHMEM_FITS((0 + (4 * nth)));
    KPR_SET_MAX_DYN_SHMEM(
        Klas_HReduce_reduce_u32_plus_kernel_3, (0 + (4 * nth)));
    KPR_KCALL(Klas_HReduce_reduce_u32_plus_kernel_3, ((uint32_t) 1U), nth,
        (0 + (4 * nth)), s, lena, a, nth, out0);
    KPR_MUST_stream_sync(s);
    KPR_MUST_stream_destroy(s);
    uint32_t *local_out = (uint32_t *) malloc(1 * sizeof(uint32_t));
    if (local_out == NULL) {
        abort();
    }
    for (size_t _ci1 = 0; _ci1 < (size_t) 1; _ci1++) {
        local_out[_ci1] = 0;
    }
    KPR_MEMCPY_D2H(local_out, out0, (((uint32_t) 4U) * ((uint32_t) 1U)));
    uint32_t res = local_out[0];
    free(local_out);
    KPR_GPU_FREE(out0);
    return res;
}

uint64_t Klas_HReduce_reduce_u64_plus(uint32_t nth, uint32_t lena, uint64_t *a)
{
    uint64_t lena64 = (uint64_t) lena;
    uint64_t nth64 = (uint64_t) nth;
    KPR_ASSERT((!((lena64 + nth64) < lena64)));
    uint8_t *tmp = (uint8_t *) KPR_GPU_ALLOC(((uint32_t) 8U), ((uint32_t) 1U));
    uint64_t *out0 = (uint64_t *) tmp;
    cudaStream_t s = KPR_FRESH_STREAM();
    KPR_SHMEM_FITS((0 + (8 * nth)));
    KPR_SET_MAX_DYN_SHMEM(
        Klas_HReduce_reduce_u64_plus_kernel_4, (0 + (8 * nth)));
    KPR_KCALL(Klas_HReduce_reduce_u64_plus_kernel_4, ((uint32_t) 1U), nth,
        (0 + (8 * nth)), s, lena, a, nth, out0);
    KPR_MUST_stream_sync(s);
    KPR_MUST_stream_destroy(s);
    uint64_t *local_out = (uint64_t *) malloc(1 * sizeof(uint64_t));
    if (local_out == NULL) {
        abort();
    }
    for (size_t _ci1 = 0; _ci1 < (size_t) 1; _ci1++) {
        local_out[_ci1] = 0;
    }
    KPR_MEMCPY_D2H(local_out, out0, (((uint32_t) 8U) * ((uint32_t) 1U)));
    uint64_t res = local_out[0];
    free(local_out);
    KPR_GPU_FREE(out0);
    return res;
}
