...
void Kuiper_Softmax_softmax_f32(size_t lena, float_t *a)
{
  ...
  float_t avg = x;
  MUST(cudaFree(a_));
  KPR_KCALL(__hoisted_2, lena, (size_t)1U, (size_t)0U, ga, avg);
  cudaDeviceSynchronize();
...
void Kuiper_Softmax_softmax_f64(size_t lena, double_t *a)
{
  ...
  double_t avg = x;
  MUST(cudaFree(a_));
  KPR_KCALL(__hoisted_5, lena, (size_t)1U, (size_t)0U, ga, avg);
  cudaDeviceSynchronize();
