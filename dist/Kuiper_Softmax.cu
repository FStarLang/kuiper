...
static void __hoisted_0(half_t *ga)
{
  size_t i = blockIdx_x();
  ga[i] = __hexp(ga[i]);
}
...
static void __hoisted_1(size_t lena, half_t *a_)
{
  ...
  while ((size_t)(1U << (uint32_t)n) < lena)
  {
    ...
    if (nextid < lena)
      if ((tid & (size_t)(1U << (uint32_t)(it + (size_t)1U)) - (size_t)1U) == (size_t)0U)
        a_[tid] = __hadd(a_[tid], a_[nextid]);
    n = it + (size_t)1U;
  }
}
...
static void __hoisted_2(half_t *ga, half_t avg)
{
  size_t i = blockIdx_x();
  ga[i] = __hdiv(ga[i], avg);
}
...
static void __hoisted_3(float_t *ga)
{
  size_t i = blockIdx_x();
  ga[i] = exp(ga[i]);
}
...
static void __hoisted_4(size_t lena, float_t *a_)
{
  size_t tid = threadIdx_x();
  size_t n = (size_t)0U;
  while ((size_t)(1U << (uint32_t)n) < lena)
...
static void __hoisted_5(float_t *ga, float_t avg)
{
  size_t i = blockIdx_x();
  ga[i] /= avg;
}
...
static void __hoisted_6(double_t *ga)
{
  size_t i = blockIdx_x();
  ga[i] = exp(ga[i]);
}
...
static void __hoisted_7(size_t lena, double_t *a_)
{
  size_t tid = threadIdx_x();
  size_t n = (size_t)0U;
  while ((size_t)(1U << (uint32_t)n) < lena)
...
static void __hoisted_8(double_t *ga, double_t avg)
{
  size_t i = blockIdx_x();
  ga[i] /= avg;
}
...
void Kuiper_Softmax_softmax_f64(size_t lena, double_t *a)
{
  ...
  KPR_KCALL(__hoisted_6, lena, (size_t)1U, (size_t)1U, (size_t)0U, ga);
  cudaDeviceSynchronize();
  double_t *a_ = (double_t *)KPR_GPU_ALLOC((size_t)8U * lena);
  MUST(cudaMemcpy(a_, ga, (size_t)8U * lena, cudaMemcpyDeviceToDevice));
  KPR_KCALL(__hoisted_7, (size_t)1U, lena, (size_t)1U, (size_t)0U, lena, a_);
  cudaDeviceSynchronize();
  ...
  double_t avg = x;
  MUST(cudaFree(a_));
  KPR_KCALL(__hoisted_8, lena, (size_t)1U, (size_t)1U, (size_t)0U, ga, avg);
  cudaDeviceSynchronize();
