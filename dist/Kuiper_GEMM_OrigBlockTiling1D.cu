...
__hoisted_0(uint32_t shared,
            uint32_t cols,
            uint32_t mshared,
            uint32_t mcols, float_t *gA4, float_t *gB4, float_t *gC4)
{
    ...
    for (; bkIdx < mshared; bkIdx++) {
        ...
        sA[threadIdx.x] =
            gA4[(mrow * 64U + threadIdx.x / 8U) * shared + __anf01 * 8U +
                threadIdx.x % 8U];
        sB[threadIdx.x] =
            gB4[(__anf01 * 8U + threadIdx.x / 64U) * cols + mcol * 64U +
                threadIdx.x % 64U];
        __syncthreads();
        uint32_t dotIdx = 0U;
                                                                ...
                                                                float_t *b)
{
    ...
    uint32_t mrows = rows / 64U;
    uint32_t mshared = shared / 8U;
    ...
    KPR_KCALL(__hoisted_0, mrows * mcols, 512U, 4096U, shared, cols, mshared,
              mcols, gA4, gB4, gC4);
                                                                ...
                                                                float_t *gC)
{
    ...
    KPR_KCALL(__hoisted_1, mrows * mcols, 512U, 4096U, alpha, beta, shared,
              cols, mshared, mcols, gA4, gB4, gC4);
    MUST(cudaDeviceSynchronize());
}
