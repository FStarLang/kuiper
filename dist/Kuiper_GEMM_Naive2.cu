                                   ...
                                   uint32_t cols, float *a, float *b)
{
    ...
    KPR_KCALL(__hoisted_0, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
...
*/
static void
__hoisted_1(uint32_t rows, uint32_t shared, uint32_t cols, double *gA,
            double *gB, double *gC)
{
    if (1024U * blockIdx.x + threadIdx.x < rows * cols) {
        uint32_t k = 0U;
                                   ...
                                   uint32_t cols, double *a, double *b)
{
    ...
    KPR_KCALL(__hoisted_1, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
                                        ...
                                        uint32_t cols, uint32_t *a, uint32_t *b)
{
    ...
    KPR_KCALL(__hoisted_2, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
                                        ...
                                        uint32_t cols, uint64_t *a, uint64_t *b)
{
    ...
    KPR_KCALL(__hoisted_3, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
                                   ...
                                   uint32_t cols, float *a, float *b)
{
    ...
    KPR_KCALL(__hoisted_4, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
    KRML_CHECK_SIZE(sizeof(float), rows * cols);
    float *c = (float *)KRML_HOST_MALLOC(sizeof(float) * (rows * cols));
                                   ...
                                   uint32_t cols, double *a, double *b)
{
    ...
    KPR_KCALL(__hoisted_5, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
    KRML_CHECK_SIZE(sizeof(double), rows * cols);
    double *c = (double *)KRML_HOST_MALLOC(sizeof(double) * (rows * cols));
                                        ...
                                        uint32_t cols, uint32_t *a, uint32_t *b)
{
    ...
    KPR_KCALL(__hoisted_6, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
    KRML_CHECK_SIZE(sizeof(uint32_t), rows * cols);
    uint32_t *c = (uint32_t *) KRML_HOST_CALLOC(rows * cols, sizeof(uint32_t));
                                        ...
                                        uint32_t cols, uint64_t *a, uint64_t *b)
{
    ...
    KPR_KCALL(__hoisted_7, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
    KRML_CHECK_SIZE(sizeof(uint64_t), rows * cols);
    uint64_t *c = (uint64_t *) KRML_HOST_CALLOC(rows * cols, sizeof(uint64_t));
...
*/
static void
__hoisted_8(uint32_t rows, uint32_t shared, uint32_t cols, float *gA, float *gB,
            float *gC)
{
    if (1024U * blockIdx.x + threadIdx.x < rows * cols) {
        uint32_t k = 0U;
                                    ...
                                    float *gA, float *gB, float *gC)
{
    ...
    KPR_KCALL(__hoisted_8, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
...
*/
static void
__hoisted_9(uint32_t rows, uint32_t shared, uint32_t cols, double *gA,
            double *gB, double *gC)
{
    if (1024U * blockIdx.x + threadIdx.x < rows * cols) {
        uint32_t k = 0U;
                                    ...
                                    double *gA, double *gB, double *gC)
{
    ...
    KPR_KCALL(__hoisted_9, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
                                    ...
                                    uint32_t *gA, uint32_t *gB, uint32_t *gC)
{
    ...
    KPR_KCALL(__hoisted_10, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
                                    ...
                                    uint64_t *gA, uint64_t *gB, uint64_t *gC)
{
    ...
    KPR_KCALL(__hoisted_11, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
                                    ...
                                    float *gA, float *gB, float *gC)
{
    ...
    KPR_KCALL(__hoisted_12, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
                                    ...
                                    double *gA, double *gB, double *gC)
{
    ...
    KPR_KCALL(__hoisted_13, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
                                    ...
                                    uint32_t *gA, uint32_t *gB, uint32_t *gC)
{
    ...
    KPR_KCALL(__hoisted_14, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
                                    ...
                                    uint64_t *gA, uint64_t *gB, uint64_t *gC)
{
    ...
    KPR_KCALL(__hoisted_15, (rows * cols + 1023U) / 1024U, 1024U, 0U, rows,
              shared, cols, gA, gB, gC);
    MUST(cudaDeviceSynchronize());
}
