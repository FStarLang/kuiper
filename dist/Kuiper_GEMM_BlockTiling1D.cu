...
)
{
  ...
  while (bk < mshared)
  {
    ...
    while (i0 < (size_t)32U)
    {
      ...
      sa1[vi * (size_t)32U + threadIdx.x] =
        gA[(blockIdx.x / mcols * (size_t)32U + vi) * shared + vbk1 * (size_t)32U + threadIdx.x];
      sa2[vi * (size_t)32U + threadIdx.x] =
        gB[(vbk1 * (size_t)32U + vi) * cols + blockIdx.x % mcols * (size_t)32U + threadIdx.x];
      i0 += (size_t)1U;
    }
    __syncthreads();
    size_t sk = (size_t)0U;
  ...
  for (; row < (size_t)32U; row += (size_t)1U)
    tileC[(blockIdx.x / mcols * (size_t)32U + row) * cols +
      blockIdx.x % mcols * (size_t)32U + threadIdx.x]
    = sums[row];
}
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    while (i0 < (size_t)32U)
    {
      size_t vi = i0;
      sa1[vi * (size_t)32U + threadIdx.x] =
        gA[(vbk1 * (size_t)32U + threadIdx.x) * rows + blockIdx.x / mcols * (size_t)32U + vi];
      sa2[vi * (size_t)32U + threadIdx.x] =
        gB[(blockIdx.x % mcols * (size_t)32U + threadIdx.x) * shared + vbk1 * (size_t)32U + vi];
      i0 += (size_t)1U;
    }
    __syncthreads();
    size_t sk = (size_t)0U;
  ...
  size_t row = (size_t)0U;
  for (; row < (size_t)32U; row += (size_t)1U)
    tileC[(blockIdx.x % mcols * (size_t)32U + threadIdx.x) * rows +
      blockIdx.x / mcols * (size_t)32U + row]
    = sums[row];
}
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    while (i0 < (size_t)16U)
    {
      ...
      sa1[vi * (size_t)16U + threadIdx.x] =
        gA[(blockIdx.x / mcols * (size_t)16U + vi) * shared + vbk1 * (size_t)16U + threadIdx.x];
      sa2[vi * (size_t)16U + threadIdx.x] =
        gB[(vbk1 * (size_t)16U + vi) * cols + blockIdx.x % mcols * (size_t)16U + threadIdx.x];
      i0 += (size_t)1U;
    }
    __syncthreads();
    size_t sk = (size_t)0U;
  ...
  float_t *tileC = gC;
  size_t row = (size_t)0U;
  for (; row < (size_t)16U; row += (size_t)1U)
    tileC[(blockIdx.x / mcols * (size_t)16U + row) * cols +
      blockIdx.x % mcols * (size_t)16U + threadIdx.x]
    = sums[row];
}
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    while (i0 < (size_t)16U)
    {
      size_t vi = i0;
      sa1[vi * (size_t)16U + threadIdx.x] =
        gA[(vbk1 * (size_t)16U + threadIdx.x) * rows + blockIdx.x / mcols * (size_t)16U + vi];
      sa2[vi * (size_t)16U + threadIdx.x] =
        gB[(blockIdx.x % mcols * (size_t)16U + threadIdx.x) * shared + vbk1 * (size_t)16U + vi];
      i0 += (size_t)1U;
    }
    __syncthreads();
    size_t sk = (size_t)0U;
  ...
  size_t row = (size_t)0U;
  for (; row < (size_t)16U; row += (size_t)1U)
    tileC[(blockIdx.x % mcols * (size_t)16U + threadIdx.x) * rows +
      blockIdx.x / mcols * (size_t)16U + row]
    = sums[row];
}
