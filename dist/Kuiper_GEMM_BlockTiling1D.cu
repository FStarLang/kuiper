...
)
{
  ...
  for (; bk < mshared; bk += 1U)
  {
    ...
    for (; i0 < 32U; i0 += 1U)
    {
      ...
      sa1[vi * 32U + threadIdx.x] =
        gA[(blockIdx.x / mcols * 32U + vi) * shared + vbk1 * 32U + threadIdx.x];
      sa2[vi * 32U + threadIdx.x] =
        gB[(vbk1 * 32U + vi) * cols + blockIdx.x % mcols * 32U + threadIdx.x];
    }
    __syncthreads();
    uint32_t sk = 0U;
    for (; sk < 32U; sk += 1U)
  ...
  uint32_t row = 0U;
  for (; row < 32U; row += 1U)
    tileC[(blockIdx.x / mcols * 32U + row) * cols + blockIdx.x % mcols * 32U + threadIdx.x] =
      sums[row];
}
...
)
{
  ...
  for (; bk < mshared; bk += 1U)
  {
    ...
    for (; i0 < 32U; i0 += 1U)
    {
      uint32_t vi = i0;
      sa1[vi * 32U + threadIdx.x] =
        gA[(vbk1 * 32U + threadIdx.x) * rows + blockIdx.x / mcols * 32U + vi];
      sa2[vi * 32U + threadIdx.x] =
        gB[(blockIdx.x % mcols * 32U + threadIdx.x) * shared + vbk1 * 32U + vi];
    }
    __syncthreads();
    uint32_t sk = 0U;
    for (; sk < 32U; sk += 1U)
  ...
  uint32_t row = 0U;
  for (; row < 32U; row += 1U)
    tileC[(blockIdx.x % mcols * 32U + threadIdx.x) * rows + blockIdx.x / mcols * 32U + row] =
      sums[row];
}
...
)
{
  ...
  for (; bk < mshared; bk += 1U)
  {
    ...
    for (; i0 < 16U; i0 += 1U)
    {
      ...
      sa1[vi * 16U + threadIdx.x] =
        gA[(blockIdx.x / mcols * 16U + vi) * shared + vbk1 * 16U + threadIdx.x];
      sa2[vi * 16U + threadIdx.x] =
        gB[(vbk1 * 16U + vi) * cols + blockIdx.x % mcols * 16U + threadIdx.x];
    }
    __syncthreads();
    uint32_t sk = 0U;
    for (; sk < 16U; sk += 1U)
  ...
  uint32_t row = 0U;
  for (; row < 16U; row += 1U)
    tileC[(blockIdx.x / mcols * 16U + row) * cols + blockIdx.x % mcols * 16U + threadIdx.x] =
      sums[row];
}
...
)
{
  ...
  for (; bk < mshared; bk += 1U)
  {
    ...
    for (; i0 < 16U; i0 += 1U)
    {
      uint32_t vi = i0;
      sa1[vi * 16U + threadIdx.x] =
        gA[(vbk1 * 16U + threadIdx.x) * rows + blockIdx.x / mcols * 16U + vi];
      sa2[vi * 16U + threadIdx.x] =
        gB[(blockIdx.x % mcols * 16U + threadIdx.x) * shared + vbk1 * 16U + vi];
    }
    __syncthreads();
    uint32_t sk = 0U;
    for (; sk < 16U; sk += 1U)
  ...
  uint32_t row = 0U;
  for (; row < 16U; row += 1U)
    tileC[(blockIdx.x % mcols * 16U + threadIdx.x) * rows + blockIdx.x / mcols * 16U + row] =
      sums[row];
}
...
)
{
  ...
  for (; row < 32U; row += 1U)
    tileC[(blockIdx.x / mcols * 32U + row) * cols + blockIdx.x % mcols * 32U + threadIdx.x] =
      beta * tileC[(blockIdx.x / mcols * 32U + row) * cols + blockIdx.x % mcols * 32U + threadIdx.x]
      + alpha * sums[row];
}
...
)
{
  ...
  for (; row < 32U; row += 1U)
    tileC[(blockIdx.x % mcols * 32U + threadIdx.x) * rows + blockIdx.x / mcols * 32U + row] =
      beta * tileC[(blockIdx.x % mcols * 32U + threadIdx.x) * rows + blockIdx.x / mcols * 32U + row]
      + alpha * sums[row];
}
...
)
{
  ...
  for (; row < 16U; row += 1U)
    tileC[(blockIdx.x / mcols * 16U + row) * cols + blockIdx.x % mcols * 16U + threadIdx.x] =
      beta * tileC[(blockIdx.x / mcols * 16U + row) * cols + blockIdx.x % mcols * 16U + threadIdx.x]
      + alpha * sums[row];
}
...
)
{
  ...
  for (; row < 16U; row += 1U)
    tileC[(blockIdx.x % mcols * 16U + threadIdx.x) * rows + blockIdx.x / mcols * 16U + row] =
      beta * tileC[(blockIdx.x % mcols * 16U + threadIdx.x) * rows + blockIdx.x / mcols * 16U + row]
      + alpha * sums[row];
}
