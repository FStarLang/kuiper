...
{
  float_t *sa1 = (float_t *)KPR_SHMEM_AT((uint32_t)0U);
  float_t *sa2 = (float_t *)KPR_SHMEM_AT((uint32_t)4096U);
  float_t sums[32U];
  memset(sums, 0U, (uint32_t)32U * sizeof (float_t));
  ...
  while (bk < mshared)
  {
    ...
    while (i0 < (uint32_t)32U)
    {
      uint32_t vi = i0;
      sa1[vi * (uint32_t)32U + threadIdx.x] =
        gA[(blockIdx.x / mcols * (uint32_t)32U + vi) * shared + vbk1 * (uint32_t)32U + threadIdx.x];
      sa2[vi * (uint32_t)32U + threadIdx.x] =
        gB[(vbk1 * (uint32_t)32U + vi) * cols + blockIdx.x % mcols * (uint32_t)32U + threadIdx.x];
      i0 += (uint32_t)1U;
    }
    __syncthreads();
    uint32_t sk = (uint32_t)0U;
    while (sk < (uint32_t)32U)
    {
      uint32_t i = (uint32_t)0U;
      float_t v2 = sa2[sk * (uint32_t)32U + threadIdx.x];
      for (; i < (uint32_t)32U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)32U + sk] * v2;
      sk += (uint32_t)1U;
    }
    bk += (uint32_t)1U;
  }
  float_t *tileC = gC;
  uint32_t row = (uint32_t)0U;
  for (; row < (uint32_t)32U; row += (uint32_t)1U)
    tileC[(blockIdx.x / mcols * (uint32_t)32U + row) * cols +
      blockIdx.x % mcols * (uint32_t)32U + threadIdx.x]
    = sums[row];
}
...
{
  double_t *sa1 = (double_t *)KPR_SHMEM_AT((uint32_t)0U);
  double_t *sa2 = (double_t *)KPR_SHMEM_AT((uint32_t)8192U);
  double_t sums[32U];
  memset(sums, 0U, (uint32_t)32U * sizeof (double_t));
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
  {
    __syncthreads();
    ...
    {
      uint32_t i = (uint32_t)0U;
      double_t v2 = sa2[sk * (uint32_t)32U + threadIdx.x];
      for (; i < (uint32_t)32U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)32U + sk] * v2;
      sk += (uint32_t)1U;
    }
...
{
  uint32_t *sa1 = (uint32_t *)KPR_SHMEM_AT((uint32_t)0U);
  uint32_t *sa2 = (uint32_t *)KPR_SHMEM_AT((uint32_t)4096U);
  uint32_t sums[32U];
  memset(sums, 0U, (uint32_t)32U * sizeof (uint32_t));
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
  {
    __syncthreads();
    ...
    {
      uint32_t i = (uint32_t)0U;
      uint32_t v2 = sa2[sk * (uint32_t)32U + threadIdx.x];
      for (; i < (uint32_t)32U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)32U + sk] * v2;
      sk += (uint32_t)1U;
    }
...
{
  uint64_t *sa1 = (uint64_t *)KPR_SHMEM_AT((uint32_t)0U);
  uint64_t *sa2 = (uint64_t *)KPR_SHMEM_AT((uint32_t)8192U);
  uint64_t sums[32U];
  memset(sums, 0U, (uint32_t)32U * sizeof (uint64_t));
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
  {
    __syncthreads();
    ...
    {
      uint32_t i = (uint32_t)0U;
      uint64_t v2 = sa2[sk * (uint32_t)32U + threadIdx.x];
      for (; i < (uint32_t)32U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)32U + sk] * v2;
      sk += (uint32_t)1U;
    }
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    while (i0 < (uint32_t)32U)
    {
      uint32_t vi = i0;
      sa1[vi * (uint32_t)32U + threadIdx.x] =
        gA[(vbk1 * (uint32_t)32U + threadIdx.x) * rows + blockIdx.x / mcols * (uint32_t)32U + vi];
      sa2[vi * (uint32_t)32U + threadIdx.x] =
        gB[(blockIdx.x % mcols * (uint32_t)32U + threadIdx.x) * shared + vbk1 * (uint32_t)32U + vi];
      i0 += (uint32_t)1U;
    }
    __syncthreads();
    uint32_t sk = (uint32_t)0U;
  ...
  uint32_t row = (uint32_t)0U;
  for (; row < (uint32_t)32U; row += (uint32_t)1U)
    tileC[(blockIdx.x % mcols * (uint32_t)32U + threadIdx.x) * rows +
      blockIdx.x / mcols * (uint32_t)32U + row]
    = sums[row];
}
...
{
  float_t *sa1 = (float_t *)KPR_SHMEM_AT((uint32_t)0U);
  float_t *sa2 = (float_t *)KPR_SHMEM_AT((uint32_t)1024U);
  float_t sums[16U];
  memset(sums, 0U, (uint32_t)16U * sizeof (float_t));
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
  {
    __syncthreads();
    uint32_t vbk1 = bk;
    uint32_t i0 = (uint32_t)0U;
    while (i0 < (uint32_t)16U)
    {
      uint32_t vi = i0;
      sa1[vi * (uint32_t)16U + threadIdx.x] =
        gA[(blockIdx.x / mcols * (uint32_t)16U + vi) * shared + vbk1 * (uint32_t)16U + threadIdx.x];
      sa2[vi * (uint32_t)16U + threadIdx.x] =
        gB[(vbk1 * (uint32_t)16U + vi) * cols + blockIdx.x % mcols * (uint32_t)16U + threadIdx.x];
      i0 += (uint32_t)1U;
    }
    __syncthreads();
    uint32_t sk = (uint32_t)0U;
    while (sk < (uint32_t)16U)
    {
      uint32_t i = (uint32_t)0U;
      float_t v2 = sa2[sk * (uint32_t)16U + threadIdx.x];
      for (; i < (uint32_t)16U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)16U + sk] * v2;
      sk += (uint32_t)1U;
    }
    bk += (uint32_t)1U;
  }
  float_t *tileC = gC;
  uint32_t row = (uint32_t)0U;
  for (; row < (uint32_t)16U; row += (uint32_t)1U)
    tileC[(blockIdx.x / mcols * (uint32_t)16U + row) * cols +
      blockIdx.x % mcols * (uint32_t)16U + threadIdx.x]
    = sums[row];
}
...
{
  double_t *sa1 = (double_t *)KPR_SHMEM_AT((uint32_t)0U);
  double_t *sa2 = (double_t *)KPR_SHMEM_AT((uint32_t)2048U);
  double_t sums[16U];
  memset(sums, 0U, (uint32_t)16U * sizeof (double_t));
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
  {
    __syncthreads();
    ...
    {
      uint32_t i = (uint32_t)0U;
      double_t v2 = sa2[sk * (uint32_t)16U + threadIdx.x];
      for (; i < (uint32_t)16U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)16U + sk] * v2;
      sk += (uint32_t)1U;
    }
...
{
  uint32_t *sa1 = (uint32_t *)KPR_SHMEM_AT((uint32_t)0U);
  uint32_t *sa2 = (uint32_t *)KPR_SHMEM_AT((uint32_t)1024U);
  uint32_t sums[16U];
  memset(sums, 0U, (uint32_t)16U * sizeof (uint32_t));
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
  {
    __syncthreads();
    ...
    {
      uint32_t i = (uint32_t)0U;
      uint32_t v2 = sa2[sk * (uint32_t)16U + threadIdx.x];
      for (; i < (uint32_t)16U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)16U + sk] * v2;
      sk += (uint32_t)1U;
    }
...
{
  uint64_t *sa1 = (uint64_t *)KPR_SHMEM_AT((uint32_t)0U);
  uint64_t *sa2 = (uint64_t *)KPR_SHMEM_AT((uint32_t)2048U);
  uint64_t sums[16U];
  memset(sums, 0U, (uint32_t)16U * sizeof (uint64_t));
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
  {
    __syncthreads();
    ...
    {
      uint32_t i = (uint32_t)0U;
      uint64_t v2 = sa2[sk * (uint32_t)16U + threadIdx.x];
      for (; i < (uint32_t)16U; i += (uint32_t)1U)
        sums[i] += sa1[i * (uint32_t)16U + sk] * v2;
      sk += (uint32_t)1U;
    }
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    while (i0 < (uint32_t)16U)
    {
      uint32_t vi = i0;
      sa1[vi * (uint32_t)16U + threadIdx.x] =
        gA[(vbk1 * (uint32_t)16U + threadIdx.x) * rows + blockIdx.x / mcols * (uint32_t)16U + vi];
      sa2[vi * (uint32_t)16U + threadIdx.x] =
        gB[(blockIdx.x % mcols * (uint32_t)16U + threadIdx.x) * shared + vbk1 * (uint32_t)16U + vi];
      i0 += (uint32_t)1U;
    }
    __syncthreads();
    uint32_t sk = (uint32_t)0U;
  ...
  uint32_t row = (uint32_t)0U;
  for (; row < (uint32_t)16U; row += (uint32_t)1U)
    tileC[(blockIdx.x % mcols * (uint32_t)16U + threadIdx.x) * rows +
      blockIdx.x / mcols * (uint32_t)16U + row]
    = sums[row];
}
