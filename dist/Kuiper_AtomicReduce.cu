...
{
  uint32_t r = 0U;
  uint32_t *gr = (uint32_t *)KPR_GPU_ALLOC((uint32_t)4U, (uint32_t)1U);
  MUST(cudaMemcpy(gr, &r, (uint32_t)4U, cudaMemcpyHostToDevice));
  KPR_KCALL(__hoisted_0, n, (uint32_t)1U, (uint32_t)0U, a, gr);
...
uint64_t Kuiper_AtomicReduce_reduce_u64(uint32_t n, uint64_t *a)
{
  ...
  KPR_KCALL(__hoisted_1, n, (uint32_t)1U, (uint32_t)0U, a, gr);
  cudaDeviceSynchronize();
...
float_t Kuiper_AtomicReduce_reduce_f32(uint32_t n, float_t *a)
{
  ...
  MUST(cudaMemcpy(gr, &r, (uint32_t)4U, cudaMemcpyHostToDevice));
  KPR_KCALL(__hoisted_2, n, (uint32_t)1U, (uint32_t)0U, a, gr);
  cudaDeviceSynchronize();
  MUST(cudaMemcpy(&r, gr, (uint32_t)4U, cudaMemcpyDeviceToHost));
  MUST(cudaFree(gr));
  return r;
...
double_t Kuiper_AtomicReduce_reduce_f64(uint32_t n, double_t *a)
{
  ...
  double_t *gr = (double_t *)KPR_GPU_ALLOC((uint32_t)8U, (uint32_t)1U);
  MUST(cudaMemcpy(gr, &r, (uint32_t)8U, cudaMemcpyHostToDevice));
  KPR_KCALL(__hoisted_3, n, (uint32_t)1U, (uint32_t)0U, a, gr);
  cudaDeviceSynchronize();
  MUST(cudaMemcpy(&r, gr, (uint32_t)8U, cudaMemcpyDeviceToHost));
  MUST(cudaFree(gr));
  return r;
