            ...
            float *gA, float *gB, float *gC, uint32_t mshared, uint32_t mcols)
{
    ...
    for (; bk < mshared; bk++) {
        ...
        float
         v1 =
            gA[(blockIdx.x / mcols * tile + threadIdx.x / tile) * shared +
               vbk * tile + threadIdx.x % tile];
        float
         v2 =
            gB[(vbk * tile + threadIdx.x / tile) * cols +
        ...
        for (; k < tile; k++)
            sum1 +=
                sa1[threadIdx.x / tile * tile + k] * sa2[k * tile +
                                                         threadIdx.x % tile];
        float t = sum1;
        sum += t;
    }
                                  ...
                                  uint32_t cols, float *a, float *b)
{
    ...
    KPR_KCALL(__hoisted_0,
              ...
              tile * tile,
              4U * (tile * tile) + 4U * (tile * tile),
              tile, shared, cols, gA, gB, gC, mshared, mcols);
...
uint64_t
    * Kuiper_GEMM_SHMem_matmul_u64_rrr(uint32_t tile,
                                       uint32_t rows,
                                       uint32_t shared,
            ...
            float *gA, float *gB, float *gC, uint32_t mshared, uint32_t mcols)
{
    ...
    for (; bk < mshared; bk++) {
        ...
        float
         v1 =
            gA[(vbk * tile + threadIdx.x % tile) * rows +
               blockIdx.x / mcols * tile + threadIdx.x / tile];
        float
         v2 =
            gB[(blockIdx.x % mcols * tile + threadIdx.x % tile) * shared +
               vbk * tile + threadIdx.x / tile];
        __syncthreads();
        sa1[threadIdx.x] = v1;
        sa2[threadIdx.x] = v2;
                                  ...
                                  uint32_t cols, float *a, float *b)
{
    ...
    KPR_KCALL(__hoisted_4,
              ...
              tile * tile,
              4U * (tile * tile) + 4U * (tile * tile),
              tile, rows, shared, gA, gB, gC, mshared, mcols);
    MUST(cudaDeviceSynchronize());
    KRML_CHECK_SIZE(sizeof(float), rows * cols);
    float *c = (float *)KRML_HOST_MALLOC(sizeof(float) * (rows * cols));
    if (c != NULL)
...
uint64_t
    * Kuiper_GEMM_SHMem_matmul_u64_ccc(uint32_t tile,
                                       uint32_t rows,
                                       uint32_t shared,
                                       uint32_t cols, uint64_t *a, uint64_t *b)
{
            ...
            float *gA, float *gB, float *gC, uint32_t mshared, uint32_t mcols)
{
    ...
    for (; bk < mshared; bk++) {
        ...
        float
         v1 =
            gA[(blockIdx.x / mcols * 32U + threadIdx.x / 32U) * shared +
               vbk * 32U + threadIdx.x % 32U];
        float
         v2 =
            gB[(vbk * 32U + threadIdx.x / 32U) * cols +
        ...
        for (; k < 32U; k++)
            sum1 +=
                sa1[threadIdx.x / 32U * 32U + k] * sa2[k * 32U +
                                                       threadIdx.x % 32U];
        float t = sum1;
        sum += t;
    }
...
uint64_t
    * Kuiper_GEMM_SHMem_matmul_u64_tile32_rrr(uint32_t rows,
                                              uint32_t shared,
                                              uint32_t cols,
             ...
             float *gA, float *gB, float *gC, uint32_t mshared, uint32_t mcols)
{
    ...
    for (; bk < mshared; bk++) {
        ...
        float
         v1 =
            gA[(vbk * 32U + threadIdx.x % 32U) * rows +
               blockIdx.x / mcols * 32U + threadIdx.x / 32U];
        float
         v2 =
            gB[(blockIdx.x % mcols * 32U + threadIdx.x % 32U) * shared +
               vbk * 32U + threadIdx.x / 32U];
        __syncthreads();
        sa1[threadIdx.x] = v1;
        sa2[threadIdx.x] = v2;
...
uint64_t
    * Kuiper_GEMM_SHMem_matmul_u64_tile32_ccc(uint32_t rows,
                                              uint32_t shared,
                                              uint32_t cols,
                                              uint64_t *a, uint64_t *b)
{
             ...
             float *gA, float *gB, float *gC, uint32_t mshared, uint32_t mcols)
{
    ...
    for (; bk < mshared; bk++) {
        ...
        float
         v1 =
            gA[(blockIdx.x / mcols * 16U + threadIdx.x / 16U) * shared +
               vbk * 16U + threadIdx.x % 16U];
        float
         v2 =
            gB[(vbk * 16U + threadIdx.x / 16U) * cols +
        ...
        for (; k < 16U; k++)
            sum1 +=
                sa1[threadIdx.x / 16U * 16U + k] * sa2[k * 16U +
                                                       threadIdx.x % 16U];
        float t = sum1;
        sum += t;
    }
...
uint64_t
    * Kuiper_GEMM_SHMem_matmul_u64_tile16_rrr(uint32_t rows,
                                              uint32_t shared,
                                              uint32_t cols,
                                              uint64_t *a, uint64_t *b)
{
             ...
             float *gA, float *gB, float *gC, uint32_t mshared, uint32_t mcols)
{
    ...
    for (; bk < mshared; bk++) {
        ...
        float
         v1 =
            gA[(vbk * 16U + threadIdx.x % 16U) * rows +
               blockIdx.x / mcols * 16U + threadIdx.x / 16U];
        float
         v2 =
            gB[(blockIdx.x % mcols * 16U + threadIdx.x % 16U) * shared +
               vbk * 16U + threadIdx.x / 16U];
        __syncthreads();
        sa1[threadIdx.x] = v1;
        sa2[threadIdx.x] = v2;
...
uint64_t
    * Kuiper_GEMM_SHMem_matmul_u64_tile16_ccc(uint32_t rows,
                                              uint32_t shared,
                                              uint32_t cols,
                                              uint64_t *a, uint64_t *b)
{
...
void
Kuiper_GEMM_SHMem_g_matmul_f32_ccc(uint32_t tile,
                                   uint32_t rows,
                                   uint32_t shared,
                                   uint32_t cols,
                                   float *gA, float *gB, float *gC)
...
void
Kuiper_GEMM_SHMem_g_matmul_u64_ccc(uint32_t tile,
                                   uint32_t rows,
                                   uint32_t shared,
                                   uint32_t cols,
                                   uint64_t *gA, uint64_t *gB, uint64_t *gC)
...
void
Kuiper_GEMM_SHMem_g_matmul_u64_tile32_rrr(uint32_t rows,
                                          uint32_t shared,
                                          uint32_t cols,
                                          uint64_t *gA,
...
void
Kuiper_GEMM_SHMem_g_matmul_u64_tile32_ccc(uint32_t rows,
                                          uint32_t shared,
                                          uint32_t cols,
                                          uint64_t *gA,
                                          uint64_t *gB, uint64_t *gC)
...
void
Kuiper_GEMM_SHMem_g_matmul_u64_tile16_rrr(uint32_t rows,
                                          uint32_t shared,
                                          uint32_t cols,
                                          uint64_t *gA,
                                          uint64_t *gB, uint64_t *gC)
...
void
Kuiper_GEMM_SHMem_g_matmul_u64_tile16_ccc(uint32_t rows,
                                          uint32_t shared,
                                          uint32_t cols,
                                          uint64_t *gA,
                                          uint64_t *gB, uint64_t *gC)
...
Kuiper_GEMM_SHMem_g_gemm_f32_rrr(uint32_t tile,
                                 ...
                                 uint32_t rows,
                                 uint32_t shared,
                                 uint32_t cols, float *gA, float *gB, float *gC)
{
    KPR_ASSERT(tile > 0U);
    KPR_GUARD(rows % tile == 0U);
    KPR_GUARD(shared % tile == 0U);
    ...
    KPR_KCALL(__hoisted_48,
              ...
              tile * tile,
              4U * (tile * tile) + 4U * (tile * tile),
              tile, alpha, beta, shared, cols, gA, gB, gC, mshared, mcols);
    MUST(cudaDeviceSynchronize());
}
                                 ...
                                 uint32_t cols, float *gA, float *gB, float *gC)
{
    ...
    KPR_KCALL(__hoisted_52,
              ...
              tile * tile,
              4U * (tile * tile) + 4U * (tile * tile),
              tile, alpha, beta, rows, shared, gA, gB, gC, mshared, mcols);
    MUST(cudaDeviceSynchronize());
}
...
void
Kuiper_GEMM_SHMem_g_gemm_u64_ccc(uint32_t tile,
                                 uint64_t alpha,
                                 uint64_t beta,
                                 uint32_t rows,
                                 uint32_t shared,
...
void
Kuiper_GEMM_SHMem_g_gemm_u64_tile32_ccc(uint64_t alpha,
                                        uint64_t beta,
                                        uint32_t rows,
                                        uint32_t shared,
                                        uint32_t cols,
...
void
Kuiper_GEMM_SHMem_g_gemm_u64_tile16_ccc(uint64_t alpha,
                                        uint64_t beta,
                                        uint32_t rows,
                                        uint32_t shared,
                                        uint32_t cols,
