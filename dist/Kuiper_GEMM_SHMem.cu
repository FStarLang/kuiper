...
)
{
  ...
  float_t *sa2 = (float_t *)KPR_SHMEM_AT((uint32_t)4U * (tile * tile));
  float_t *gTile = gC;
...
)
{
  ...
  KPR_KCALL(__hoisted_0,
    mrows * mcols,
    tile * tile,
    (uint32_t)4U * (tile * tile) + (uint32_t)4U * (tile * tile),
    tile,
    shared,
    cols,
    gA,
...
)
{
  ...
  double_t *sa2 = (double_t *)KPR_SHMEM_AT((uint32_t)8U * (tile * tile));
  double_t *gTile = gC;
...
)
{
  ...
  KPR_KCALL(__hoisted_1,
    mrows * mcols,
    tile * tile,
    (uint32_t)8U * (tile * tile) + (uint32_t)8U * (tile * tile),
    tile,
    shared,
    cols,
    gA,
...
)
{
  ...
  uint32_t *sa2 = (uint32_t *)KPR_SHMEM_AT((uint32_t)4U * (tile * tile));
  uint32_t *gTile = gC;
  uint32_t sum = 0U;
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
...
)
{
  ...
  uint64_t *sa2 = (uint64_t *)KPR_SHMEM_AT((uint32_t)8U * (tile * tile));
  uint64_t *gTile = gC;
  uint64_t sum = 0ULL;
  uint32_t bk = (uint32_t)0U;
  while (bk < mshared)
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    float_t
    v1 =
      gA[(blockIdx.x / mcols * (uint32_t)32U + threadIdx.x / (uint32_t)32U) * shared +
        vbk * (uint32_t)32U + threadIdx.x % (uint32_t)32U];
    float_t
    v2 =
      gB[(vbk * (uint32_t)32U + threadIdx.x / (uint32_t)32U) * cols +
    ...
    for (; k < (uint32_t)32U; k += (uint32_t)1U)
      sum1 +=
        sa1[threadIdx.x / (uint32_t)32U * (uint32_t)32U + k] *
          sa2[k * (uint32_t)32U + threadIdx.x % (uint32_t)32U];
    float_t t = sum1;
    sum += t;
    bk = vbk + (uint32_t)1U;
  }
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    float_t
    v1 =
      gA[(vbk * (uint32_t)32U + threadIdx.x % (uint32_t)32U) * rows +
        blockIdx.x / mcols * (uint32_t)32U + threadIdx.x / (uint32_t)32U];
    float_t
    v2 =
      gB[(blockIdx.x % mcols * (uint32_t)32U + threadIdx.x % (uint32_t)32U) * shared +
        vbk * (uint32_t)32U + threadIdx.x / (uint32_t)32U];
    __syncthreads();
    sa1[threadIdx.x] = v1;
    sa2[threadIdx.x] = v2;
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    float_t
    v1 =
      gA[(blockIdx.x / mcols * (uint32_t)16U + threadIdx.x / (uint32_t)16U) * shared +
        vbk * (uint32_t)16U + threadIdx.x % (uint32_t)16U];
    float_t
    v2 =
      gB[(vbk * (uint32_t)16U + threadIdx.x / (uint32_t)16U) * cols +
    ...
    for (; k < (uint32_t)16U; k += (uint32_t)1U)
      sum1 +=
        sa1[threadIdx.x / (uint32_t)16U * (uint32_t)16U + k] *
          sa2[k * (uint32_t)16U + threadIdx.x % (uint32_t)16U];
    float_t t = sum1;
    sum += t;
    bk = vbk + (uint32_t)1U;
  }
...
)
{
  ...
  while (bk < mshared)
  {
    ...
    float_t
    v1 =
      gA[(vbk * (uint32_t)16U + threadIdx.x % (uint32_t)16U) * rows +
        blockIdx.x / mcols * (uint32_t)16U + threadIdx.x / (uint32_t)16U];
    float_t
    v2 =
      gB[(blockIdx.x % mcols * (uint32_t)16U + threadIdx.x % (uint32_t)16U) * shared +
        vbk * (uint32_t)16U + threadIdx.x / (uint32_t)16U];
    __syncthreads();
    sa1[threadIdx.x] = v1;
    sa2[threadIdx.x] = v2;
