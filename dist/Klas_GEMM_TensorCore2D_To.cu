...
__hoisted_g_gemm_bf16_f32_bf16_64x64x16_16x16x16_2x2_0(
    ...
    __nv_bfloat16 *gC, __nv_bfloat16 *gD, float alpha, float beta,
    uint32_t nthr)
{
    ...
    for (; bkIdx < num_k_tiles; bkIdx++) {
        ...
        for (; i < 1024U; i += 1024U) {
            ...
            vec_memcpy(local, tileB + (cols * __anf0 * 16U + mcol * 64U +
                                       cols * row + col));
            uint32_t k = 0U;
            for (; k < 8U; k++)
    ...
    uint32_t nthr)
{
    ...
    for (; bkIdx < num_k_tiles; bkIdx++) {
        ...
        for (; i < 2048U; i += 1024U) {
            ...
            uint32_t row = (i + threadIdx.x * 8U) / 64U;
            uint32_t col = (i + threadIdx.x * 8U) % 64U;
            vec_memcpy(local, tileB + (cols * __anf0 * 32U + mcol * 64U +
                                       cols * row + col));
            uint32_t k = 0U;
            for (; k < 8U; k++)
                sB[row * 64U + col + k] = local[k];
    ...
    uint32_t nthr)
{
    ...
    for (; bkIdx < num_k_tiles; bkIdx++) {
        ...
        for (; i < 4096U; i += 1024U) {
            ...
            uint32_t row = (i + threadIdx.x * 8U) / 64U;
            uint32_t col = (i + threadIdx.x * 8U) % 64U;
            vec_memcpy(local, tileB + (cols * __anf0 * 64U + mcol * 64U +
                                       cols * row + col));
            uint32_t k = 0U;
            for (; k < 8U; k++)
                sB[row * 64U + col + k] = local[k];
    ...
    uint32_t nthr)
{
    ...
    for (; bkIdx < num_k_tiles; bkIdx++) {
        ...
        for (; i < 2048U; i += 1024U) {
            ...
            vec_memcpy(local, tileB + (cols * __anf0 * 16U + mcol * 128U +
                                       cols * row + col));
            uint32_t k = 0U;
            for (; k < 8U; k++)
    ...
    uint32_t nthr)
{
    ...
    for (; bkIdx < num_k_tiles; bkIdx++) {
        ...
        for (; i < 4096U; i += 2048U) {
            ...
            uint32_t row = (i + threadIdx.x * 8U) / 128U;
            uint32_t col = (i + threadIdx.x * 8U) % 128U;
            vec_memcpy(local, tileB + (cols * __anf0 * 32U + mcol * 128U +
                                       cols * row + col));
            uint32_t k = 0U;
            for (; k < 8U; k++)
                sB[row * 128U + col + k] = local[k];
    ...
    uint32_t nthr)
{
    ...
    for (; bkIdx < num_k_tiles; bkIdx++) {
        ...
        for (; i < 8192U; i += 2048U) {
            ...
            uint32_t row = (i + threadIdx.x * 8U) / 128U;
            uint32_t col = (i + threadIdx.x * 8U) % 128U;
            vec_memcpy(local, tileB + (cols * __anf0 * 64U + mcol * 128U +
                                       cols * row + col));
            uint32_t k = 0U;
            for (; k < 8U; k++)
                sB[row * 128U + col + k] = local[k];
