...
(* The runtime entry the rule's emitted call names.  Nothing in the source
   calls it, so the rule must [register_root] it. *)
[@@FStar.Attributes.custard_extern "KPR_KCALL";
   FStar.Attributes.custard_c_header "kuiper.h"]
assume val kcall (k : unit -> FStar.All.ML unit) (nblk nthr smem : Kuiper.SizeT.t)
