  ...
  (row: nat{row < rows}) (col: nat{col < columns}) (to: nat)
    : Lemma
      ...
      (ensures (
        ...
        matmul_single rows shared columns s1 s2 row col to) = U64.add_mod (U64.mul_mod (Seq.Base.index s1 (row * shared + (to - 1))) (Seq.Base.index s2 (col + (to - 1) * columns))) (matmul_single rows shared columns s1 s2 row col (to - 1)
      ))
  = ()
#pop-options
...
let lemma_matmul_index
  ...
  (idx: nat{idx < rows * columns})
    : Lemma
      (ensures (Seq.Base.index (matmul rows shared columns s1 s2) idx) == matmul_single rows shared columns s1 s2 (idx / columns) (idx % columns) shared)
  = ()
