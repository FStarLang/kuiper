...
unfold
let kpre #et (size:nat) (ga1 ga2 r : larray et size) (tid:nat) : slprop =
  gpu_pts_to_array1 ga1 tid **
  gpu_pts_to_array1 ga2 tid **
  gpu_pts_to_array1 r   tid

unfold
let kpost #et (size:nat) (ga1 ga2 r : larray et size) (tid:nat) : slprop =
  gpu_pts_to_array1 ga1 tid **
  gpu_pts_to_array1 ga2 tid **
  gpu_pts_to_array1 r   tid
    ...
    thread_id size tid
{
  ...
  pts_to_slice_ref ga1 _ _;
  pts_to_slice_ref ga2 _ _;

  slice_write r tid (slice_read ga1 tid `mul` slice_read ga2 tid);
  ...
  ensures  forall+ (i: natlt sz). gpu_pts_to_array1 arr #f i
{
  ...
  forevery_map
    (fun (i: natlt sz) ->
      pts_to_slice arr #f i (i + 1) seq![Seq.Base.index v i])
  ...
  ensures exists* (v : seq a). arr |-> Frac f v
{
  forevery_map
    (fun (i: natlt sz) -> gpu_pts_to_array1 arr #f i)
    (fun (i: natlt sz) -> exists* v. pts_to_slice arr #f i (i + 1) seq![v])
    fn i {
      unfold gpu_pts_to_array1 arr #f i; with v. _;
      pts_to_slice_ref arr i (i+1);
      assert pure (v `Seq.equal` seq![Seq.index v 0]);
      rewrite each v as seq![Seq.index v 0];
    };
  let v = forevery_exists (fun (i: natlt sz) v ->
    pts_to_slice arr #f i (i + 1) seq![v]);
  let v' = Seq.init_ghost sz v;
  forevery_ext
    (fun (i: natlt sz) -> pts_to_slice arr #f i (i + 1) seq![v i])
    (fun (i: natlt sz) -> pts_to_slice arr #f i (i + 1) seq![Seq.index v' i]);
  array_unslice_1 arr;
}

ghost
fn block_setup
  (#et:Type) (ga1 ga2 gr : larray et size)
  ()
  norewrite
  requires
...
let kdesc (#et:Type) {| scalar et |}
    (ga1:larray et size{ is_global_array ga1 })
    (ga2:larray et size{ is_global_array ga2 })
    (r : larray et size{ is_global_array r })
  : kernel_desc
