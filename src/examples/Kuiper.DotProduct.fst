...
fn kernel
  ...
  (ga1 ga2 : gpu_array elem_t (reveal size))
  (r : gpu_array elem_t (reveal size))
  ...
  ensures  cpu
{
  ...
  while (let v = !i; (v `SZ.op_Less_Hat` m_size))
     invariant b.
       exists* v. pts_to i v **
       (exists* (s:seq u32). (a1 |-> s) ** pure (len s == size)) **
       (exists* (s:seq u32). (a2 |-> s) ** pure (len s == size)) **
       pure (b == (SZ.v v < size))
  {
    let v = !i;
  ...
  while (let v = !i; (v `SZ.op_Less_Hat` m_size))
     invariant b. exists* vi vpsum.
       pts_to i vi **
       pts_to psum vpsum  **
       (exists* (s : seq u32). (ar |-> s) ** pure (len s == size)) **
       pure (b == (SZ.v vi < size))
  {
    let vi = !i;
    let ri = ar.(vi);
