...
let kpre_pair (rows shared columns: nat)
  ...
  (#s1: erased (Seq.seq U64.t))
  (#s2: erased (Seq.seq U64.t))
  (size: erased pos)
  : slprop
  =
...
let kpost (shared: nat)
  ...
  (ga2: gpu_array U64.t (shared * columns))
  (r: gpu_array U64.t (rows * columns))
  (#s1: erased (Seq.seq U64.t) {Seq.length s1 == rows * shared})
  (#s2: erased (Seq.seq U64.t) {Seq.length s2 == shared * columns})
...
ghost fn fold_pre
  ...
  (#s1: erased (Seq.seq U64.t) {Seq.length s1 == rows * shared})
  (#s2: erased (Seq.seq U64.t) {Seq.length s2 == shared * columns})
  (#sr: (Seq.seq U64.t) {Seq.length sr == 1})
  (size: erased pos { (reveal size <: nat) == rows * columns })
  (idx : nat)
