...
let singleton #a (elem: a) : Seq.seq a = Seq.cons elem Seq.empty

let rec matmul_single
  (rows shared columns: nat)
  (s1: (Seq.seq U64.t){ Seq.length s1 == rows * shared })
  (s2 : (Seq.seq U64.t){ Seq.length s2 == shared * columns })
  ...
  (row: nat{row < rows}) (col: nat{col < columns}) (to: nat)
    : Lemma
      ...
      (ensures (
        ...
        reveal (matmul_single rows shared columns s1 s2 row col to) = U64.add_mod (U64.mul_mod (Seq.index s1 (row * shared + (to - 1))) (Seq.index s2 (col + (to - 1) * columns))) (matmul_single rows shared columns s1 s2 row col (to - 1))
      ))
  = ()
#pop-options
...
let matmul
  ...
  (s1: (Seq.seq U64.t){ Seq.length s1 == rows * shared })
  (s2 : (Seq.seq U64.t){ Seq.length s2 == shared * columns })
    : GTot (sr:(Seq.seq U64.t){ Seq.length sr == rows * columns })
  =
