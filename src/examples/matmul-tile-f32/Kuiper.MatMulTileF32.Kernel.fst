    ...
    bigstar 0 nthr (Barrier.barrier_mm nthr ar it tid)
{
  ...
  let s = !sum;
  sum := Kuiper.Float32.(add (mul ga1_val ga2_val) s);
}
       ...
       ** mbarrier_tok nthr (Barrier.barrier_mm nthr ar) (2*iv + 2) tid
{
  ...
  gpu_slice_share_underspec #0 #f32 #smem_sz ar #1.0R (2 * tid) (2 * tid + 2) nthr;
  ...
  gpu_slice_gather_underspec #0 #f32 #smem_sz ar #1.0R (2 * tid) (2 * tid + 2) nthr;
}
         ...
         (tid_to_idx rows shared columns bdim (thread_index etid))
{
  ...
  let mut i = 0sz;
  let mut sum = Kuiper.Float32.zero;
  ...
  while (let iv = !i; (iv <^ shared_tile))
     invariant b.
       ...
       gpu
       ** (exists* sumv. pts_to sum sumv)
       ** Impure.gpu_pts_to_matrix #f32 rows shared ga1 (SZ.v nblk * SZ.v nthr) s1
       ** Impure.gpu_pts_to_matrix #f32 shared columns ga2 (SZ.v nblk * SZ.v nthr) s2
       ** (exists* s. gpu_pts_to_array_slice ar (2 * tid) (2 * tid + 2) s)
