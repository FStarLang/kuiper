...
ghost fn transfer_barrier_mm (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat) (from to: (i: nat { 0 <= i /\ i < nthr }))
...
ghost fn unfold_barrier_mm_odd (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 <> 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
  requires Barrier.barrier_mm nthr ar it from to
  ensures exists* v. gpu_pts_to_array_slice ar #(1.0R /. Real.of_int nthr) (2 * to) (2 * to + 2) v
{
  unfold Barrier.barrier_mm nthr ar it from to;
}
ghost fn fold_barrier_mm_odd (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 <> 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
...
ghost fn unfold_barrier_mm_even (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 == 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
  requires Barrier.barrier_mm nthr ar it from to
  ensures exists* v. gpu_pts_to_array_slice ar #(1.0R /. Real.of_int nthr) (2 * from) (2 * from + 2) v
{
  unfold Barrier.barrier_mm nthr ar it from to;
}
ghost fn fold_barrier_mm_even (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 == 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
...
fn block_setup_ghost
  ...
  (bid: sz)
  requires block_setup nthr ** (exists* v. gpu_pts_to_array #f32 #smem_sz ar #1.0R v)
...
// let split_input_a2 (shared: pos) (bidx_x: nat) (tdim_x bdim_x tdim_y bdim_shared: pos) (a2: seq f32 { FStar.Seq.length a2 == tdim_y * bdim_shared * tdim_x * bdim_x  }): seq (seq f32)
