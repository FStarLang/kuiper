module Kuiper.MatMulTileF32.Barrier
...
ghost
fn transfer_barrier_mm (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat) (from to: (i: nat { 0 <= i /\ i < nthr }))
  requires barrier_mm nthr ar it from to
  ensures  barrier_mm nthr ar (it + 1) to from
{
  unfold barrier_mm nthr ar it from to;
  fold barrier_mm nthr ar (it + 1) to from;
}

ghost
fn unfold_barrier_mm_odd (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 <> 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
  requires barrier_mm nthr ar it from to
...
ghost
fn fold_barrier_mm_odd (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 <> 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
  requires exists* v. gpu_pts_to_slice ar #(1.0R /. nthr) (2 * to) (2 * to + 2) v
  ensures barrier_mm nthr ar it from to
{
  fold barrier_mm nthr ar it from to;
}

ghost
fn unfold_barrier_mm_even (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 == 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
  requires barrier_mm nthr ar it from to
...
ghost
fn fold_barrier_mm_even (nthr: nat) (smem_sz : nat{smem_sz == 2 * nthr}) (ar: gpu_array f32 smem_sz) (it: nat{it % 2 == 0}) (from to: (i: nat { 0 <= i /\ i < nthr }))
  ...
  ensures block_setup nthr ** bigstar 0 nthr (shared_pre nthr 0 ar bid)
{
  ...
  // let vv: mseq (get_dims (from_dims seq![2 <: pos; bdim; bdim])) f32 = v;
  ...
  // rewrite each (array_to_matrix #f32 #(multiply (cons #pos 2 (cons #pos bdim (cons #pos bdim (empty #pos))))) ar) as ar_split;
  ...
  // gpu_matrix_slice_permission #f32 #(from_dims seq![2 <: pos; bdim <: pos; bdim <: pos]) ar_split 0 #vv seq![(bdim <: pos); (bdim <: pos)];
  ...
  // gpu_matrix_slice_permission #f32 #(from_dims seq![bdim; bdim]) ar1 0 seq![(bdim <: pos)];
  // gpu_matrix_slice_permission #f32 #(from_dims seq![bdim; bdim]) ar2 0 seq![(bdim <: pos)];
  ...
  //           (slice #(get_dims (from_dims seq![bdim; bdim])) #f32
  //               (slice #(get_dims (from_dims seq![2 <: pos; bdim; bdim])) #f32 vv 0 0 seq![(bdim <: pos); (bdim <: pos)]) 0 i seq![(bdim <: pos)]) **
  //         gpu_pts_to_matrix (slice_matrix ar2
  //               i
  //               (Seq.Base.index (from_dims seq![bdim; bdim]) 0)._2)
  //           (remove (from_dims seq![bdim; bdim]) 0)
  //           (slice #(get_dims (from_dims seq![bdim; bdim])) #f32
  //               (slice #(get_dims (from_dims seq![2 <: pos; bdim; bdim])) #f32 vv 0 1 seq![(bdim <: pos); (bdim <: pos)]) 0 i seq![(bdim <: pos)])));
  ...
  // ghost fn slice_again (ari1 ari2: gpu_matrix f32)
  ...
  // // let t: Type u#0 = (gpu_matrix f32 seq![bdim; bdim; 2] <: Type u#0);

  // // assert (pure (multiply seq![2 <: pos; bdim; bdim] == 2 * bdim * bdim));
  // // let ar_split = (to_gpu_matrix seq![2 <: pos; bdim; bdim] ar) <: gpu_matrix f32 seq![2 <: pos; bdim; bdim];
  // // FStar.Seq.lemma_eq_intro (remove seq![2 <: pos; bdim; bdim] 0) seq![bdim; bdim];
  // // assert (pure (remove seq![2 <: pos; bdim; bdim] 0 == seq![bdim; bdim] /\ gpu_matrix f32 (remove seq![2 <: pos; bdim; bdim] 0) == gpu_matrix f32 seq![bdim; bdim]));
  // // let ar1 = slice_matrix ar_split 0 0 <: gpu_matrix f32 (remove seq![2 <: pos; bdim; bdim] 0);
  
  // // let ar11 = (*coerce_eq ()*) ar1 <: gpu_matrix f32 seq![bdim; bdim];
...
// let split_input_a2 (shared: pos) (bidx_x: nat) (tdim_x bdim_x tdim_y bdim_shared: pos) (a2: seq f32 { FStar.len a2 == tdim_y * bdim_shared * tdim_x * bdim_x  }): seq (seq f32)
//   = FStar.Seq.init bdim_shared (fun bidx_y -> FStar.Seq.init (tdim_x * tdim_y) (fun tidx -> ))

// let shared_post (nthr : sz { 0 < nthr /\ nthr <= max_threads }) (ar: gpu_array f32 SZ.(2sz *^ nthr)) (i: nat { 0 <= i /\ i < nthr }): slprop =
