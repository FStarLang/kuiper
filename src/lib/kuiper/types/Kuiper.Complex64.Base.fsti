module Kuiper.Complex64.Base

(* All assumptions about double-precision complex (cuDoubleComplex / float2).

   Mirrors the abstract treatment of Kuiper.Float64.Base: [t] is opaque, mapped
   by the extraction plugin to CUDA's [cuDoubleComplex]; the arithmetic maps to
...
open Kuiper.Scalars.Base   (* add, mul, zero, one, eq for the f64 component *)
open Kuiper.Floating.Base  (* sub, div for the f64 component *)
module F = Kuiper.Float64   (* the public f64 type and its floating instance *)

new
val t : Type0

(* Real / imaginary components and the constructor (make_cuDoubleComplex). *)
val re : t -> F.t
val im : t -> F.t
val cmk : F.t -> F.t -> t
...
(* ----------------------------------------------------------------------- *)
(* Model laws (right-hand sides use the f64 component operations).           *)
(* ----------------------------------------------------------------------- *)
