...
(* General (fused-map, multi-type) natively batched approximate shared-memory
...
(* Natively batched approximate shared-memory GEMM: a single launch computes
...
(* General (fused-map, multi-type) approximate shared-memory GEMM.  Derived from
   [gbmmcomb_gpu_approx] at [batch = 1] via a single-page (rank-3) relayout of
   the rank-2 matrices, so there is a single kernel description in this module
   (the batched one). *)
inline_for_extraction noextract
