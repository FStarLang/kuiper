...
(* Taking a subtile commutes with the pointwise chest combination.  Keep this
   definitional equality out of [epilogue_loop_step]'s large Pulse context:
   unfolding both chests there makes the otherwise routine fold time out. *)
let subtile_acc2
  (#et : Type)
  (#rows #cols : nat)
...
let chest_comb_acc2
  ...
  (j : natlt cols)
  : Lemma (
      acc2 (chest_comb comb c1 c2) i j
      == comb (acc2 c1 i j) (acc2 c2 i j))
= Kuiper.Chest.acc_pat
    (fun (ij : abs (rows @| cols @| INil)) ->
      comb (Kuiper.Chest.acc c1 ij) (Kuiper.Chest.acc c2 ij))
    (i, (j, ()))

let chest_comb_subtile
  (#rows #cols : nat)
  (comb : binop real)
  (c1 c2 : chest2 real rows cols)
  (trows : pos { trows /?+ rows })
  (tcols : pos { tcols /?+ cols })
  (tr : natlt (rows / trows))
  (tc : natlt (cols / tcols))
  : Lemma (
      ematrix_subtile (chest_comb comb c1 c2) trows tcols tr tc
      == chest_comb comb
          (ematrix_subtile c1 trows tcols tr tc)
          (ematrix_subtile c2 trows tcols tr tc))
= let lhs =
    ematrix_subtile (chest_comb comb c1 c2) trows tcols tr tc in
  let c1tile = ematrix_subtile c1 trows tcols tr tc in
  let c2tile = ematrix_subtile c2 trows tcols tr tc in
  let rhs = chest_comb comb c1tile c2tile in
  let aux (i : natlt trows) (j : natlt tcols)
    : Lemma (acc2 lhs i j == acc2 rhs i j)
    = subtile_acc2 (chest_comb comb c1 c2) trows tcols tr tc i j;
      chest_comb_acc2 comb c1 c2
        (tr * trows + i) (tc * tcols + j);
      subtile_acc2 c1 trows tcols tr tc i j;
      subtile_acc2 c2 trows tcols tr tc i j;
      chest_comb_acc2 comb c1tile c2tile i j in
