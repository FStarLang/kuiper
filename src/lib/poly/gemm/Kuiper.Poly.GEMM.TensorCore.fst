      ...
      (rchProd |-> vrchProd')
{
        ...
        (rchProd |-> vrchProd)
  {
    ...
    gpu_matrix_tile gA tm 1;
    let a_tile = gpu_matrix_subtile gA tm 1sz arow !dotIdx;
    let va = gpu_matrix_read a_tile 0sz 0sz;
    // let va = gpu_matrix_read #_ #_ #_ #_ #(c_subtile_layout l1 tm 1sz arow !dotIdx) a_tile 0sz 0sz;
    admit();
    ...
    gpu_matrix_tile gB 1 tn;
    let b_tile = gpu_matrix_subtile gB 1 tn !dotIdx bcol;
    ...
    while (SZ.(!i1 <^ tn))
      invariant
        exists* (vi : sz{vi <= tn}) (vrBrow : erased (lseq et tn)).
          ...
          (rBrow |-> vrBrow)
    {
      let vb = gpu_matrix_read b_tile !i1;
      rBrow.(!i1) <- vb;
...
//   (tn : szp{tn /? bn})
//   (tk : szp{tk /? bk})
//   (aFrag : fragment et FragA tm tn tk FragLCM)
...
//   while (SZ.(!dotIdx <^ bk))
//     invariant
//       exists* (vdotIdx : sz{vdotIdx <= bk}) (vaFrag : ematrix et tm tk)
      ...
      (rchProd |-> vrchProd'))
{
  ...
  let tt = thread_tile (block_tile gC bm bn bid) tm tn tid;
  admit();
  // let thread_tile = gpu_matrix_subtile
        ...
        (rchProd |-> vrchProd)
  {
          ...
          (rchProd |-> vrchProd)
    {
      ...
      with i0 j0 v0.
      rewrite gpu_matrix_pts_to_cell gC i0 j0 v0
          as gpu_matrix_pts_to_cell gC vresIdxM vresIdxN v0;
      let v0 = gpu_matrix_read_cell gC vresIdxM vresIdxN;
      ...
      let v' = comb v0 v1;
      gpu_matrix_write_cell gC vresIdxM vresIdxN v';
