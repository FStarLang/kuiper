...
val barrier_matrix (nth: nat) (sr gr : gpu_array ety nth) (v: seq ety { Seq.length v == nth }) (it: nat) (from to : (i: nat { i < nth }))
  : slprop
...
[@@pulse_unfold]
let shared_pre (nth: nat) (sr gr : gpu_array ety nth) (s : erased (seq ety))
  (#_: squash (Seq.length s == nth)) (it: nat) (tid:nat{tid < nth})
  : slprop =
    (exists* s. gpu_pts_to_array_slice sr tid (tid+1) s) **
    mbarrier_tok nth (barrier_matrix nth sr gr s) it tid

[@@pulse_unfold]
let shared_post (nth: nat) (sr gr : gpu_array ety nth) (s : erased (seq ety))
  (#_: squash (Seq.length s == nth)) (tid:nat{tid < nth})
  : slprop =
    kpost nth sr s tid **
