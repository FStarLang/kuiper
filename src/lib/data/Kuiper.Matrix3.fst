...
let gpu_array3 (et:Type0) (#d0 #d1 #d2 : nat) (l : mlayout3 d0 d1 d2) : Type0 =
  A.varray (aview3_from_mlayout3 et l)

let is_global_array3 (#et:Type0) (#d0 #d1 #d2 : nat) (#l : mlayout3 d0 d1 d2) (a : gpu_array3 et l) : prop =
  A.is_global_varray a

let from_array3 (#et : Type0) (#d0 #d1 #d2 : erased nat) (l : mlayout3 d0 d1 d2) (a : gpu_array et (mlayout3_size l)) : gpu_array3 et l =
  A.from_array (aview3_from_mlayout3 et l) a

let core3 (#et : Type0) (#d0 #d1 #d2 : erased nat) (#l : mlayout3 d0 d1 d2) (a : gpu_array3 et l) : gpu_array et (mlayout3_size l) =
  A.core a

let lem_core3_from_array3 #et #d0 #d1 #d2 #l a = ()
let lem_from_array3_core3 #et #d0 #d1 #d2 l p = ()

let gpu_array3_pts_to
...
instance is_send_across_global_array3
...
{
  unfold gpu_array3_pts_to a #f v;
  let r = A.varray_read a (i, j, k);
  fold gpu_array3_pts_to a #f v;
  r
}
...
{
  unfold gpu_array3_pts_to a v;
  A.varray_write a (i, j, k) r;
  fold gpu_array3_pts_to a (mk3 (fun i' j' k' ->
    if i'=i && j'=j && k'=k then r else acc3 v i' j' k'));
}

(* ============ CELL-LEVEL ============ *)

let gpu_array3_pts_to_cell
  (#et:Type) (#d0 #d1 #d2 : nat) (#l : mlayout3 d0 d1 d2)
  ([@@@mkey] a : gpu_array3 et l)
  (#[T.exact (`1.0R)] f : perm)
      ...
      gpu_array3_pts_to_cell a #f i j k (acc3 v i j k)
{
  ...
  fn aux (ijk : natlt d0 & natlt d1 & natlt d2)
    ...
    ensures  gpu_array3_pts_to_cell a #f ijk._1 ijk._2 ijk._3 (acc3 v ijk._1 ijk._2 ijk._3)
  {
    rewrite each ijk as (ijk._1, ijk._2, ijk._3);
    fold gpu_array3_pts_to_cell a #f ijk._1 ijk._2 ijk._3 (acc3 v ijk._1 ijk._2 ijk._3);
  };
    ...
    a |-> Frac f v
{
  forevery_flatten3'
    (fun (ijk : natlt d0 & natlt d1 & natlt d2) ->
      gpu_array3_pts_to_cell a #f ijk._1 ijk._2 ijk._3 (acc3 v ijk._1 ijk._2 ijk._3));
  forevery_ext
    (fun (ijk : natlt d0 & natlt d1 & natlt d2) ->
      gpu_array3_pts_to_cell a #f ijk._1 ijk._2 ijk._3 (acc3 v ijk._1 ijk._2 ijk._3))
    (fun (ijk : natlt d0 & natlt d1 & natlt d2) ->
...
fn gpu_array3_extract_slice0
  ...
  ensures
    factored
      ...
      (a |-> Frac f v)
{
  gpu_array3_pts_to_ref a;
  gpu_array3_explode a;
  ...
  fn to_mat_cell (j : natlt d1) (k : natlt d2)
    requires gpu_array3_pts_to_cell a #f (reveal i) j k (acc3 v (reveal i) j k)
    ensures  gpu_matrix_pts_to_cell (slice0_matrix a i) #f j k (macc (earray3_slice0 v i) j k)
  {
    ...
    rewrite gpu_array3_pts_to_cell a #f (reveal i) j k (acc3 v (reveal i) j k)
  ...
  fn restore ()
    ...
    requires
      forall+ (x : natlt d0 { ~(eq2 #(natlt d0) x (reveal i)) }).
        forall+ (j : natlt d1) (k : natlt d2).
          gpu_array3_pts_to_cell a #f x j k (acc3 v x j k)
    requires
      slice0_matrix a i |-> Frac f (earray3_slice0 v i)
    ensures
      a |-> Frac f v
  {
    ...
    fn from_mat_cell (j : natlt d1) (k : natlt d2)
      ...
      ensures  gpu_array3_pts_to_cell a #f (reveal i) j k (acc3 v (reveal i) j k)
    {
      slice0_cell_eq a (reveal i) j k f (acc3 v (reveal i) j k);
      rewrite gpu_matrix_pts_to_cell (slice0_matrix a i) #f j k (macc (earray3_slice0 v i) j k)
           as gpu_array3_pts_to_cell a #f (reveal i) j k (acc3 v (reveal i) j k);
    };
      ...
      (a |-> Frac f v)
{
  ...
  fn extract_j_inner (i : natlt d0)
    ...
    ensures
      (forall+ (k : natlt d2).
        gpu_array3_pts_to_cell a #f i (reveal j) k (acc3 v i (reveal j) k)) **
      (forall+ (x : natlt d1 { ~(eq2 #(natlt d1) x (reveal j)) }) (k : natlt d2).
        gpu_array3_pts_to_cell a #f i x k (acc3 v i x k))
  {
  ...
  fn to_mat_cell (i : natlt d0) (k : natlt d2)
    requires gpu_array3_pts_to_cell a #f i (reveal j) k (acc3 v i (reveal j) k)
    ensures  gpu_matrix_pts_to_cell (slice1_matrix a j) #f i k (macc (earray3_slice1 v j) i k)
  {
    ...
    rewrite gpu_array3_pts_to_cell a #f i (reveal j) k (acc3 v i (reveal j) k)
    ...
    ensures a |-> Frac f v
  {
    ...
    fn from_mat_cell (i : natlt d0) (k : natlt d2)
      ...
      ensures  gpu_array3_pts_to_cell a #f i (reveal j) k (acc3 v i (reveal j) k)
    {
      slice1_cell_eq a (reveal j) i k f (acc3 v i (reveal j) k);
      rewrite gpu_matrix_pts_to_cell (slice1_matrix a j) #f i k (macc (earray3_slice1 v j) i k)
           as gpu_array3_pts_to_cell a #f i (reveal j) k (acc3 v i (reveal j) k);
    };
    forevery_map_2 _ _ from_mat_cell;
          ...
          gpu_array3_pts_to_cell a #f i j' k (acc3 v i j' k)
    {
      forevery_insert
        ...
        (fun (j' : natlt d1) ->
          forall+ (k : natlt d2). gpu_array3_pts_to_cell a #f i j' k (acc3 v i j' k))
        (reveal j);
      forevery_unrefine _;
    };
      ...
      (a |-> Frac f v)
{
  ...
  fn extract_k_inner (i : natlt d0) (j : natlt d1)
    ...
    requires
      forall+ (k' : natlt d2).
        gpu_array3_pts_to_cell a #f i j k' (acc3 v i j k')
    ensures
      gpu_array3_pts_to_cell a #f i j (reveal k) (acc3 v i j (reveal k)) **
      (forall+ (x : natlt d2 { ~(eq2 #(natlt d2) x (reveal k)) }).
        gpu_array3_pts_to_cell a #f i j x (acc3 v i j x))
  {
  ...
  fn to_mat_cell (i : natlt d0) (j : natlt d1)
    requires gpu_array3_pts_to_cell a #f i j (reveal k) (acc3 v i j (reveal k))
    ensures  gpu_matrix_pts_to_cell (slice2_matrix a k) #f i j (macc (earray3_slice2 v k) i j)
  {
    ...
    rewrite gpu_array3_pts_to_cell a #f i j (reveal k) (acc3 v i j (reveal k))
    ...
    ensures a |-> Frac f v
  {
    ...
    fn from_mat_cell (i : natlt d0) (j : natlt d1)
      ...
      ensures  gpu_array3_pts_to_cell a #f i j (reveal k) (acc3 v i j (reveal k))
    {
      slice2_cell_eq a (reveal k) i j f (acc3 v i j (reveal k));
      rewrite gpu_matrix_pts_to_cell (slice2_matrix a k) #f i j (macc (earray3_slice2 v k) i j)
           as gpu_array3_pts_to_cell a #f i j (reveal k) (acc3 v i j (reveal k));
    };
    forevery_map_2 _ _ from_mat_cell;
          ...
          gpu_array3_pts_to_cell a #f i j k' (acc3 v i j k')
    {
      forevery_insert
        ...
        (fun (k' : natlt d2) -> gpu_array3_pts_to_cell a #f i j k' (acc3 v i j k'))
        (reveal k);
      forevery_unrefine _;
    };
