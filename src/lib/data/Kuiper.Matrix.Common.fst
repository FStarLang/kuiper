...
let bij (rows cols : nat) : (natlt rows & (natlt cols & unit) =~ natlt rows & natlt cols) =
  {
    ff = (fun (i, (j, ())) -> (i, j));
    gg = (fun (i, j) -> (i, (j, ())));
    ff_gg = ez;
...
let to_seq (#et:Type) (#rows #cols : nat)
  ...
  = Seq.init_ghost (rows * cols) (fun i ->
      let r, c = Kuiper.Injection.inverse_f l.map i in
      macc m r c)
...
let open Kuiper.Bijection in
{
  iview = {
    len = l.len;
    ait = natlt rows & (natlt cols & unit);
    step = {
      imap = inj_bij (bij rows cols) `Kuiper.Injection.inj_comp` l.map;
    };
  };
  ctn = solve;
}
...
let from_seq_rel (#et #rows #cols : _) (l : mlayout rows cols {is_full_layout l})
  ...
  = assert (Kuiper.Chest.equal (from_seq l s) (V.from_seq (aview_from_mlayout et l) s))
