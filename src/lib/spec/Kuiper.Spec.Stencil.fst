...
let neighborhood_at
  (#et : Type0)
  (#rows #cols : nat)
  (eIn: ematrix et (rows + 2) (cols + 2))
  (i: nat{i > 0 /\ i <= rows})
  (j: nat{j > 0 /\ j <= cols})
  : GTot (seq et)
  =
  seq![
    (macc eIn (i - 1) (j - 1)); (macc eIn (i - 1) j); (macc eIn (i - 1) (j + 1));
    (macc eIn i (j - 1)); (macc eIn i j); (macc eIn i (j + 1));
    (macc eIn (i + 1) (j - 1)); (macc eIn (i + 1) j); (macc eIn (i + 1) (j + 1))
  ]
...
let stencil_result
  ...
  =
  mkM <| fun i j -> stencil_result_at_idx stencil eIn i j
