...
let rec msingle_approx
  ...
  = if to > 0 then begin
      ...
      Kuiper.Approximates.Base.a_add
        (MS.__matmul_single eA eB i j (to - 1)) (mul (acc2 eA i (to - 1)) (acc2 eB (to - 1) j))
        (MS.__matmul_single rA rB i j (to - 1)) (mul (acc2 rA i (to - 1)) (acc2 rB (to - 1) j))
    end else ()
#pop-options
...
let tpose_result
  ...
  (eB : chest2 et k n)
  : chest2 et n m
  = mk2 (fun i j -> MS.matmul_single eA eB j i)
...
let tpose_result_approx
  ...
  = introduce forall (i : natlt n) (j : natlt m).
        acc2 (tpose_result eA eB) i j %~ acc2 (mtranspose (MS.matmul rA rB)) i j
    with (
...
let tpartial
  ...
  (g : nat)
  : chest2 et n m
  = mk2 (fun i j -> if i * m + j < g then MS.matmul_single eA eB j i else acc2 eC i j)
...
let tpartial_zero
  ...
  = introduce forall (i : natlt n) (j : natlt m).
        acc2 (tpartial eA eB eC 0) i j == acc2 eC i j
    with ();
...
let tpartial_full
  ...
  = introduce forall (i : natlt n) (j : natlt m).
        acc2 (tpartial eA eB eC (n * m)) i j == acc2 (tpose_result eA eB) i j
    with (
...
let tpartial_step
  ...
  : Lemma
      ...
      (ensures
        upd2 (tpartial eA eB eC g) (g / m) (g % m)
  ...
  = let r : natlt n = g / m in
    ...
    introduce forall (i : natlt n) (j : natlt m).
        acc2 (upd2 (tpartial eA eB eC g) r c (MS.matmul_single eA eB c r)) i j
        == acc2 (tpartial eA eB eC (g + 1)) i j
    with (
      if i = r && j = c then ()
      else begin
        Math.Lemmas.euclidean_division_definition g m;
    ...
    assert (equal (upd2 (tpartial eA eB eC g) r c (MS.matmul_single eA eB c r))
                  (tpartial eA eB eC (g + 1)))
#pop-options
    ...
    gC |-> tpose_result eA eB
{
    ...
    decreases (v (n *^ m) - v !g)
  {
      ...
      decreases (v k - v !idx)
    {
      ...
      let av = M.tensor_read gA (M.cidx2 col i0);
      let bv = M.tensor_read gB (M.cidx2 i0 row);
