...
(* cuBLAS zaxpy: y := alpha*x + y over double-precision complex.
   ...
   Kuiper.Complex64. Because Kuiper.Complex64.Base.t is a [scalar], the generic
   kernel goes through unchanged and extracts to CUDA's cuDoubleComplex
   arithmetic (cuCadd / cuCmul). It demonstrates "complex BLAS for free". *)

#lang-pulse
open Kuiper
open Kuiper.Array1
open Kuiper.Tensor.Layout.Alg { l1_forward }
open Kuiper.Complex64           (* brings cf64 and its scalar instance *)
module Map = Kuiper.Kernel.Map
module C = Kuiper.Complex64.Base

(* Functional spec: elementwise y[i] := alpha * x[i] + y[i] (complex). *)
let s_zaxpy (#n:nat) (alpha:C.t) (sx sy : lseq C.t n) : GTot (lseq C.t n)
...
fn zaxpy_gen
  ...
  ensures  on gpu_loc (y |-> s_zaxpy alpha sx sy)
{
