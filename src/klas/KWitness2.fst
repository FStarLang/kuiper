...
let rec rdot
  ...
  = if from >= shared then zero
    else add (rdot eA eB i j (from + 1)) (mul (macc eA i from) (macc eB from j))
...
let rdot_step
  ...
  : Lemma (rdot eA eB i j from ==
           add (rdot eA eB i j (from + 1)) (mul (macc eA i from) (macc eB from j)))
  = ()
#pop-options
...
let rec rdot_approx
  ...
  = if from < shared then begin
      ...
      Kuiper.Approximates.Base.a_mul
        (macc eA i from) (macc eB from j) (macc rA i from) (macc rB from j);
      Kuiper.Approximates.Base.a_add
        (rdot eA eB i j (from + 1)) (mul (macc eA i from) (macc eB from j))
        (rdot rA rB i j (from + 1)) (mul (macc rA i from) (macc rB from j))
    end else ()
#pop-options
...
let rev_result_approx
  ...
  = introduce forall (i : natlt rows) (j : natlt cols).
        macc (rev_result eA eB) i j %~ macc (MS.matmul rA rB) i j
    with (
...
let partial
  ...
  (g : nat)
  : ematrix et rows cols
  = mkM (fun i j -> if i * cols + j < g then rdot eA eB i j 0 else macc eC i j)
...
let partial_full
  ...
  = introduce forall (i : natlt rows) (j : natlt cols).
        macc (partial eA eB eC (rows * cols)) i j == macc (rev_result eA eB) i j
    with (
...
let partial_step
  ...
  = let r : natlt rows = g / cols in
    ...
    introduce forall (i : natlt rows) (j : natlt cols).
        macc (mupd (partial eA eB eC g) r c (rdot eA eB r c 0)) i j
    ...
    assert (equal (mupd (partial eA eB eC g) r c (rdot eA eB r c 0))
                  (partial eA eB eC (g + 1)))
#pop-options
...
(* cit_fits is a (non-unfold) let, so it stays folded in queries; this intro
   lemma exposes it from the index bounds. *)
    ...
    gC |-> rev_result eA eB
{
    ...
    decreases (v (m *^ n) - v !g)
  {
      ...
      decreases (v !idx)
    {
      ...
      cit_fits_intro (v m) (v k) row i0;
      cit_fits_intro (v k) (v n) i0 col;
      let av = M.read gA (row, i0);
      let bv = M.read gB (i0, col);
