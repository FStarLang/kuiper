...
# .cu/.h themselves; shipping the checked files avoids re-verification.
mkdir -p "$PKG/obj"
...
cp -a Makefile verify.mk nvcc.mk custard.mk .common.mk .configure.mk .clang-format configure \
      fstar.sh "$PKG/"
...
# NOTE: .depend is intentionally NOT shipped: it records absolute paths from
# the build machine, so it is useless (and actively harmful) elsewhere. make
# regenerates it on first use via the rule in verify.mk.
# (.configure.output is likewise not shipped: it is host-specific and is
...
# toolchain and plugin as already up to date.
...
# never tries to rebuild the toolchain or the plugin. (.depend is not shipped
# and is regenerated by the first `make`.)
