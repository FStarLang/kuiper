...
int main(int argc, char **argv)
{
	...
	for (int l = 0; l < laps; l++) {
		...
		m3 = TIME(Kuiper_MatMul_Naive2_Inst_matmul_u64_rrr(rows, shared, columns, m1, m2), &t);
	...
	for (int l = 0; l < laps; l++) {
		...
		*/
		...
		m3 = TIME(Kuiper_MatMul_Naive2_Inst_matmul_u64_ccc(columns, shared, rows, m2, m1), &t);
		fprintf(stderr, "Estimated GIOPS: %.3f\n", (rows * shared * columns * 2.0) / t / 1e9);
	}
