...
static void cpu_matmul(
    uint32_t *A, uint32_t *B, uint32_t *C, int rows, int shared, int cols)
{
...
static void run_spmm(
    const char *label, uint32_t *AD, int rows, int shared, int cols)
{
    ...
    float t;
    TIME_void(
        Klas_SPMM_spmm_u32(rows, shared, cols, dA, drow_indices, dB, dC), &t);
    fprintf(stderr,
